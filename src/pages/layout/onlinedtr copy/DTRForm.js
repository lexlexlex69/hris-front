...
export const DTRForm = React.forwardRef((props,ref)=>{
                    ...
                    <>
                        {
                            ...
                            ?
                                ...
                                ?
                                    ...
                                    ?
                                    <td style = {{background:timein_info.status === 'DISAPPROVED' ? '#000' :'auto'}}><Tooltip arrow title={(<Box><Typography>Rectification Request details: </Typography><Typography sx={{borderTop:'solid 1px',color:'#c4c4c4',mb:1}}></Typography><Typography className ='rectification-details'>Date: {moment(timein_info.date).format('MMMM DD, YYYY')}</Typography><Typography className ='rectification-details'>Nature: {timein_info.nature}</Typography><Typography>Reason: {timein_info.reason}</Typography><Typography className ='rectification-details'>Rectified Time: {formatAMPM(timein_info.rectified_time)}</Typography><Typography className ='rectification-details'>Status: {timein_info.status}</Typography></Box>)}><EventNoteOutlinedIcon className = {timein_info.status === 'APPROVED'?'approved-rectification-icon' :'pending-rectification-icon'}/></Tooltip></td>
                                    :
    ...
    return(
        <Box sx={{p:1}} className='contextmenu'>
            ...
            <Typography sx={{fontSize:'12px',textAlign:'right',paddingRight:'100px',fontWeight:'bold',fontFamily:'Times New Roman'}}>{props.officeHead.office_head === null ?'N/A':props.officeHead.office_head.toUpperCase()}</Typography>
        ...
        <Modal
            ...
            <Box sx={style}>
                ...
                <Box sx={{m:matches?1:2,maxHeight:matches?'70vh':'auto',overflowY:matches?'scroll':'auto'}}>
