...
function WorkExp({ id: positionId }) {
    ...
    const handleRequire = async () => {
        ...
        try {
            let require = await axios.post(`/api/recruitment/toggleRequiredPointSystem`, { id: items.id, category: 'experience', isRequired: isRequired })
    ...
    const handleCreate = async () => {
        ...
        Swal.showLoading()
        try {
            let create = await axios.post(`/api/recruitment/submitEntriesForPointSystem`, { category: 'experience', id: positionId })
            Swal.close()
    ...
    const handleUpdate = async () => {
        ...
        Swal.showLoading()
        try {
            let update = await axios.post(`/api/recruitment/updateEntriesForPointSystem`, { items, category: 'experience' })
                            ...
                            <>
                                <Box sx={{ display: 'flex', gap: 2, flexDirection: 'column', mt: 2 }}>
                                    ...
                                    <Box sx={{ display: 'flex', gap: 2, flexDirection: matches ? 'column' : 'row' }}>
                                        <TextField disabled={!isRequired} label="Number of years" type='number' size='small' name='years' value={items.years} onChange={handleChange}></TextField>
