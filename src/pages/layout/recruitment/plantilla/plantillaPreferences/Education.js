...
function Education({ id: positionId }) {
    ...
    const handleRequire = async () => {
        ...
        try {
            let require = await axios.post(`/api/recruitment/toggleRequiredPointSystem`, { positionId: positionId, category: 'education', isRequired: isRequired })
    ...
    const handleUpdate = async (item) => {
        ...
        Swal.showLoading()
        let update = await axios.post(`/api/recruitment/updateEntriesForPointSystem`, { item, category: 'education' })
        Swal.close()
        if (update.data.status === 500)
        ...
        <Box sx={{ flexGrow: 1 }}>
            ...
            <Box sx={{ position: 'relative', pointerEvents: isRequired ? null : 'none' }}>
                <Box sx={{ display: 'flex', gap: 1, mb: 1 }}>
                    <CustomReactSelect apiAddress='recruitment/AutoCompletePointsystem' label='course_name' value='id' category='education' positionId={positionId} isRequired={isRequired} setForceFetchItems={setForceFetchItems} setIsRequiredDisabler={setIsRequiredDisabler} />
                </Box>
                <Box>
                    ...
                    {/* {items.length > 0 ? (
                        <Box display="" mx="" my="" sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end' }}>
                            <Button disabled={!isRequired} variant='contained' color='success' sx={{ borderRadius: '2rem' }}>set points</Button>
                        </Box>
                    ) : null} */}
