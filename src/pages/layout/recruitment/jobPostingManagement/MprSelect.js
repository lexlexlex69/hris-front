...
const MprSelect = ({ url, optionTitle, componentTitle, setTitle, defaultValue, defaultPosition }) => {
    ...
    return (
        <Autocomplete
            ...
            onChange={(e, value) => {
                console.log(value)
                if (!value) {
                    setTitle(prev => ({ ...prev, mpr_no: defaultValue }))
                }
                else {
                    setTitle(prev => ({ ...prev, mpr_no: value?.mpr_no }))
                }
