...
import { Edit, Check as CheckIcon } from '@mui/icons-material';

let mprNumberTimer = null
const Update = ({ handleClose, mprUpdate, mpr, setMpr, setTotal }) => {
    console.log(mprUpdate)
    const budgetRef = useRef()
    const [mprData, setMprData] = useState({
        casual_plantilla_id: mprUpdate?.casual_plantilla_id,
        mpr_no: mprUpdate?.mpr_no,
        mpr_id: mprUpdate?.mpr_id,
        dept_id: mprUpdate?.dept_id,
        dept_title: mprUpdate?.dept_title,
        div_id: mprUpdate?.div_id,
        sect_id: mprUpdate?.sect_id,
        position_id: mprUpdate?.position_id,
        position_name: mprUpdate?.position_name,
        date_requested: moment(mprUpdate.date_requested, "YYYY-MM-DD").format("YYYY-MM-DD"),
        date_needed: moment(mprUpdate.date_needed, "YYYY-MM-DD").format("YYYY-MM-DD"),
        budget: `${mprUpdate?.budget}`,
        head_count: mprUpdate?.head_count,
        proposed_rate: mprUpdate?.proposed_rate,
        employment_status: mprUpdate?.employment_status,
        employment_status_others: mprUpdate?.employment_status_others,
        mpr_justification: mprUpdate?.mpr_justification,
        mpr_justification_others: mprUpdate?.mpr_justification_others,
        mpr_details: mprUpdate?.mpr_details,
        mpr_duties: mprUpdate?.mpr_duties,
        mpr_qualification: mprUpdate?.mpr_qualification,
        mpr_justification: mprUpdate?.mpr_justification,
        employment_status: mprUpdate?.employment_status,
    })
    ...
    // mpr position side effect when employment status is updated
    ...
    const submitMpr = async (e) => {
        ...
        let data = {
            plantilla_casual_id: mprData.employment_status !== '1' ? '' : mprData.casual_plantilla_id,
            ...
            budget: parseInt(mprData.budget),
        ...
        let res = await axios.post(`/api/recruitment/mpr/update-mpr`, data)
        console.log(res)
        if (res.data.status === 200) {
            let mprUpdate = mpr.map((x => x.mpr_id === mprData.mpr_id ? ({
                ...x,
                mpr_no: mprData.mpr_no,
                dept_id: mprData.dept_id,
                div_id: mprData.div_id,
    ...
    const handleCheckMprNumber = async (e) => {
        ...
        setMprNumberError('')
        if (e.target.value === mprUpdate?.mpr_no) {
    ...
    useEffect(() => {
        ...
        if (firstRenderEmploymentStatus.current) {
            ...
            if (mprData.employment_status === '1') {
                setMprData(prev => ({ ...prev, proposed_rate: '', dept_id: '', position_id: '', casual_plantilla_id: '', position_name: '', dept_title: '' }))
            }
            else {
                setMprData(prev => ({ ...prev, proposed_rate: '', dept_id: prev?.dept_id, position_id: prev?.position_id, casual_plantilla_id: '', position_name: prev?.position_name, dept_title: '' }))
            }
        }
            ...
            <form onSubmit={submitMpr}>
                <Box sx={{ position: 'relative', width: '100%', p: 2 }}>
                    ...
                    {mprData.employment_status === '1' &&
                        <>
                            <Box mt={2}>
                                <MprPlantillaOfficeSelect componentTitle='PLANTILLA CASUAL REFERENCE OFFICE NAME' optionTitle='dept_title' field_id='dept_id' field_name='dept_title' defaultValue={disableDefault ? mprData.dept_title : ''} url='/api/recruitment/mpr/auto-complete-plantilla-casual-office' setTitle={setMprData} />
                            </Box>
                            <Box mt={2}>
                                <MprPositionSelect componentTitle='POSITION TITLE' optionTitle='position_name' url='/api/recruitment/mpr/auto-complete-positions-department' defaultValue={disableDefault ? mprData.position_name : ''} postData={mprData.dept_id} postDataField='dept_id' setTitle={setMprData} />
                            </Box>
                        </>
                    }
                    {mprData.employment_status !== '1' &&
                        <Box mt={2}>
                            <MprPositionSelect componentTitle='POSITION TITLE' optionTitle='position_name' url='/api/recruitment/plantilla/AutoCompletePositions' defaultValue={mprData.position_name} setTitle={setMprData} />
                        </Box>
                    }
                    ...
                    <Box display='flex' justifyContent='flex-end'>
                        <Button sx={{ mt: 2, borderRadius: '2rem' }} type='submit' variant='contained' color='warning' startIcon={<Edit />}>Update</Button>
                    </Box>
                </Box>
            </form>
        </Box >
