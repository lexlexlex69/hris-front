...
import { getAllDisapprovedRectificationRequestHistory, getAllRectificationRequestHistory } from '../DTRRequest';
...
export default function RectificationDisapprovedHistoryTable(props){
	...
	const fetchData = async page => {
		...
		const response = await axios.get(`/api/DTR/rectDisapprovedHistoryPagination?type=${props.type}&page=${page}&per_page=${perPage}`);
	...
	const handlePerRowsChange = async (newPerPage, page) => {
		...
		const response = await axios.get(`/api/DTR/rectDisapprovedHistoryPagination?type=${props.type}&page=${page}&per_page=${newPerPage}`);
    ...
    const columns = [
        ...
        {
            name:'Disapproved By',
            selector:row=>row.disapproved_by
        },
        {
            name:'Remarks',
    ...
    const refresh = async ()=>{
        ...
        Swal.showLoading()
        await axios.get(`/api/DTR/rectDisapprovedHistoryPagination?type=${props.type}&page=${1}&per_page=${perPage}`)
        .then(res=>{
    ...
    const handleProceedDownload = async (e)=>{
        ...
        const res = await getAllDisapprovedRectificationRequestHistory({from:periodFrom,to:periodTo})
        setReportsData(res.data.data)
        Swal.close();
        if(buttonRef.current !== null) {
            buttonRef.current.click();
    ...
    return(
       ...
       <Box sx={{display:'flex',justifyContent:'flex-end'}}>
        {/* <Typography sx={{fontSize:'.9rem'}}><em style={{color:'#fff',background:red[700],padding:'10px 15px 10px 10px',borderTopRightRadius:'20px',borderBottomRightRadius:'20px'}}>Disapproved Request History</em></Typography> */}
            ...
            <div style={{display:'none'}}>
                <ExcelFile element={<IconButton ref={buttonRef}></IconButton>} filename={`Rectification Reports from ${moment(periodFrom).format('MM-DD-YYYY')} to ${moment(periodTo).format('MM-DD-YYYY')}`}>
                    <ExcelSheet data={reportsData} name="Rectification Reports">
                            ...
                            <ExcelColumn label="Interval" value={(el)=>`${moment(el.rectified_at).diff(el.approved_at,'days')} day/s`}/>

                            <ExcelColumn label="Inclusive Date/s" value={(el)=>`${moment(el.created_at).format('MMMM DD, YYYY')}`}/>
                            ...
                            <ExcelColumn label="Disapproved By" value={(el)=>`${el.disapproved_by?el.disapproved_by:''}`}/>
                            <ExcelColumn label="Remarks" value={(el)=>`${el.remarks?el.remarks:''}`}/>
                        </ExcelSheet>
                </ExcelFile>
            </div>
            </Box>
            <Tooltip title='Reload disapproved data'><IconButton color='primary' className='custom-iconbutton' onClick={refresh}sx={{'&:hover':{background:blue[800],color:'#ffff'}}}><CachedOutlinedIcon/></IconButton></Tooltip>
        </Box>
