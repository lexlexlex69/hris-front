...
export const DTRForm = React.forwardRef((props,ref)=>{
    ...
    useEffect(()=>{
        ...
        console.log(props.defSignatory)
    },[props.dtrdata])
                    ...
                    <>
                        {
                            ...
                            ?
                                ...
                                ?
                                    ...
                                    <Typography className ='rectification-details'>{'Remarks: '+(timein_info.remarks?timein_info.remarks:'')} {timein_info.remarks === null && timein_info.status==='APPROVED' ? 'Pending For Rectification' :''} </Typography>
                                    {timein_info.status === 'FOR REVIEW' || timein_info.status === 'DISAPPROVED' || timein_info.remarks === 'FOR HR APPROVAL'?<Tooltip title=
                                    ...
                                    <EventNoteOutlinedIcon className = {timein_info.status === 'FOR APPROVAL'? 'approval-rectification-icon' :timein_info.status === 'APPROVED'?'approved-rectification-icon' :'pending-rectification-icon'}/>
                                    </Tooltip></td>
                                    :
                                        props.type === 'viewing'
                                        ?
                            ...
                            :
                                ...
                                :
                                <td><span style={{color:special?'red':'black'}}>{adjust_in?<strong>{formatTimeWithPeriod(time_in)}</strong>:time_in === 'OB' || time_in === 'OFT'?<strong>{time_in}</strong>:formatTimeWithPeriod(time_in)}</span>
                                </td>
                        }
                        
                        {
                            ...
                            ?
                                ...
                                ?
                                    ...
                                    <Tooltip arrow title={(
                                    <Box>
                                    <Typography>Rectification Request details: </Typography><Typography sx={{borderTop:'solid 1px',color:'#c4c4c4',mb:1}}></Typography>
                                    <Typography className ='rectification-details'>Date: {moment(breakout_info.date).format('MMMM DD, YYYY')}</Typography><Typography className ='rectification-details'>Nature: {breakout_info.nature}</Typography>
                                    <Typography>Reason: {breakout_info.reason}</Typography>
                                    <Typography className ='rectification-details'>Rectified Time: {formatAMPM(breakout_info.rectified_time)}</Typography>
                                    <Typography className ='rectification-details'>Status: {breakout_info.status}</Typography>
                                    <Typography className ='rectification-details'>{'Remarks: '+(breakout_info.remarks?breakout_info.remarks:'')} {timeout_info.remarks === null && breakout_info.status==='APPROVED' ? 'Pending For Rectification' :''} </Typography>
                                    {breakout_info.status === 'FOR REVIEW' || breakout_info.status === 'DISAPPROVED' || breakout_info.remarks === 'FOR HR APPROVAL'?<Tooltip title=
                                    ...
                                    <EventNoteOutlinedIcon className = {breakout_info.status === 'FOR APPROVAL'? 'approval-rectification-icon' :breakout_info.status === 'APPROVED'?'approved-rectification-icon' :'pending-rectification-icon'}/>
                                    </Tooltip>
                                    </td>
                                    :
                                        props.type === 'viewing'
                            ...
                            :
                                ...
                                :
                                <td><span style={{color:special?'red':'black'}}>{adjust_break_out?<strong>{formatTimeWithPeriod(break_out)}</strong>:break_out === 'OB' || break_out === 'OFT'?<strong>{break_out}</strong>:formatTimeWithPeriod(break_out)}</span></td>
                        }
    
                        {
                            ...
                            ?
                                ...
                                ?
                                    ...
                                    <Typography className ='rectification-details'>{'Remarks: '+(breakin_info.remarks?breakin_info.remarks:'')} {breakin_info.remarks === null && breakin_info.status==='APPROVED' ? 'Pending For Rectification' :''} </Typography>
                                    {breakin_info.status === 'FOR REVIEW' || breakin_info.status === 'DISAPPROVED' || breakin_info.remarks === 'FOR HR APPROVAL'?<Tooltip title=
                                    ...
                                    <EventNoteOutlinedIcon className = {breakin_info.status === 'FOR APPROVAL'? 'approval-rectification-icon' :breakin_info.status === 'APPROVED'?'approved-rectification-icon' :'pending-rectification-icon'}/>
                                    </Tooltip></td>
                                    :
                                        props.type === 'viewing'
                                        ?
                            ...
                            :
                                ...
                                :
                                <td><span style={{color:special?'red':'black'}}>{adjust_break_in?<strong>{formatTimeWithPeriod(break_in)}</strong>:break_in === 'OB' || break_in === 'OFT'?<strong>{break_in}</strong>:formatTimeWithPeriod(break_in)}</span></td>
                        }
    
                        {
                            ...
                            ?
                                ...
                                ?
                                    ...
                                    <Typography className ='rectification-details'>{'Remarks: '+(timeout_info.remarks?timeout_info.remarks:'')} {timeout_info.remarks === null && timeout_info.status==='APPROVED' ? 'Pending For Rectification' :''}</Typography>
                                    {timeout_info.status === 'FOR REVIEW' || timeout_info.status === 'DISAPPROVED' || timeout_info.remarks === 'FOR HR APPROVAL'?<Tooltip title=
                                    ...
                                    <EventNoteOutlinedIcon className = {timeout_info.status === 'FOR APPROVAL'? 'approval-rectification-icon' :timeout_info.status === 'APPROVED'?'approved-rectification-icon' :'pending-rectification-icon'}/>
                            ...
                            :
                                ...
                                :
                                <td><span style={{color:special?'red':'black'}}> {adjust_out?<strong>{formatTimeWithPeriod(time_out)}</strong>:time_out === 'OB' || time_out === 'OFT'?<strong>{time_out}</strong>:formatTimeWithPeriod(time_out)}</span> </td>
    ...
    return(
        <Box sx={{p:1}} className='contextmenu'>
            ...
            {
                props.info.head_office_designation_id
                ?
                <>
                <Typography sx={{fontSize:'12px',textAlign:'center',fontWeight:'bold',fontFamily:'Times New Roman'}}>{props.officeHead.head_name === null ?'N/A':props.officeHead.head_name.toUpperCase()}s</Typography>
