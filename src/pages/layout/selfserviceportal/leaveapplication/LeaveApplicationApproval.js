...
import { Grid, Typography,Container,Paper,Box, Button,FormControl,InputLabel,Select,MenuItem,Fade,CircularProgress,Modal, TextField,Tooltip,Skeleton,Backdrop,Stack,InputAdornment,IconButton,TableContainer,Table,TableHead,TableRow,TableBody,TableCell,TableFooter,TablePagination, Checkbox  } from '@mui/material';
//icon
...
import { getAllTypeOfLeave, getApprovalLeaveApplicationData,getFilterApprovalLeaveApplicationData,postApprovedApplicLeaveAPI,postDisApprovalApplicLeaveAPI,submitLeaveApplicationApproval } from './LeaveApplicationRequest';
...
export default function LeaveApplicationApproval(){
    ...
    const [defaultLeaveApplicationData,setDefaultLeaveApplicationData] = React.useState([])
    ...
    const [empStatus,setEmpStatus] = useState(['Permanent','Casual','COS','JO'])
        ...
        .then((response)=>{
            ...
            if(response.data){
                ...
                .then((response)=>{
                    ...
                    const unique = [...new Set(response.data.data.map(item => item.officeCode))];
                    // console.log(unique)
                    setDeptNames(unique.sort())
                    response.data.data.forEach(el=>{
                        el.selected = false;
                    })
                    setLeaveApplicationData(response.data.data)
                    setDefaultLeaveApplicationData(response.data.data)
    ...
    const submitApprovedApplication = (row)=>{
        if(row.leave_type_id === 15){
            ...
            let inc_dates1 = JSON.parse(row.inclusive_dates)
            let inc_dates_dwop = row.inclusive_dates_without_pay ==='null' || row.inclusive_dates_without_pay ===null ?[]:JSON.parse(row.inclusive_dates_without_pay);
            let dwpay = Math.round(row.days_with_pay);
            if(row.leave_type_id === 3){
                var inc_dates = inc_dates1.concat(inc_dates_dwop);
            }else{
                var inc_dates = JSON.parse(row.inclusive_dates);
            }
            let dwpay_days = [];
            let dwopay_days = inc_dates_dwop;
            let dwopay_days_text = '';
            let late_filing_arr1 = inc_dates_dwop.filter((el)=>{
                return el.late_filing;
            })
            let late_filing_arr = [];
            late_filing_arr1.forEach(el=>{
                late_filing_arr.push(el.date)
            })
            console.log(late_filing_arr1);
            console.log(late_filing_arr);
            for(var i = 0 ; i<inc_dates1.length ; i++){
                dwpay_days.push(inc_dates[i]);
            }
            if(row.inclusive_dates_without_pay !== 'null'){
                if(row.leave_type_id === 10){
                    dwopay_days_text = moment(inc_dates_dwop[0].date).format('MMMM DD-')+moment(inc_dates_dwop[inc_dates_dwop.length-1].date).format('MMMM DD ,YYYY')
                    
                }else{
                    if(row.leave_type_id === 3){
                        // inc_dates_dwop.forEach((el,key)=>{
                        // if(key === 0){
                        //     if(key === inc_dates_dwop.length-1){
                        //         if(late_filing_arr.includes(moment(el.date,'MM-DD-YYYY').format('MM-DD-YYYY'))){
                        //             dwopay_days_text+= ' - Late Filing( '+moment(el.date,'MM-DD-YYYY').format('MMMM DD, YYYY')

                        //         }else{
                        //             dwopay_days_text+= ' - WOP( '+moment(el.date,'MM-DD-YYYY').format('MMMM DD, YYYY')
                        //         }
                        //     }else{
                        //         if(late_filing_arr.includes(moment(el.date,'MM-DD-YYYY').format('MM-DD-YYYY'))){
                        //             dwopay_days_text+= ' - Late Filing( '+moment(el.date,'MM-DD-YYYY').format('MMMM DD, YYYY')+','
                        ...
                        // }else{
                        //     if(key === inc_dates_dwop.length - 1){
                        //         dwopay_days_text+= moment(el.date).format('MMMM DD, YYYY')
                        //     }else{
                        //         dwopay_days_text+= moment(el.date).format('MMMM DD, YYYY')+','
                        //     }
                        // }
                        ...
                        if(key === 0){
                            if(key === inc_dates_dwop.length-1){
                                dwopay_days_text+= ' - WOP( '+moment(el.date).format('MMMM DD, YYYY')
            ...
            console.log(temp_fin_inc_date_arr)
            temp_fin_inc_date_arr.forEach(el=>{
                console.log(el)
                if(late_filing_arr.includes(moment(el,'MM-DD-YYYY').format('MM-DD-YYYY'))){
                    dwopay_days_text = ' - Late filing ('+ moment(el,'MM-DD-YYYY').format('MMMM DD, YYYY');
                }else{
                    dwopay_days_text = ' - WOP ('+ moment(el,'MM-DD-YYYY').format('MMMM DD, YYYY');
                }
                // if(late_filing_arr1.includes(moment(el.)))
                ...
                if(date2_temp){
                    ...
                    if(moment(date1,'MM-DD-YYYY').format('MM-DD-YYYY') === moment(date2,'MM-DD-YYYY').format('MM-DD-YYYY')){
                        ...
                        fin_inc_date_arr.push({
                            ...
                            'short_name':row.short_name+' ('+total_days+' 00 00)'+(row.days_without_pay !==0?+' '+(dwopay_days_text)+')':'')
                        })
                    }else{
                        let total_days = (moment(date2,'MM-DD-YYYY').diff(moment(date1,'MM-DD-YYYY'),'days')+1)-to_sub;
            ...
            if(row.leave_type_id === 1){
                if(row.days_with_pay === .5){
                    var t_total = 0;
                    var t_fin_inc_date_arr = [];

                    fin_inc_date_arr.forEach(el=>{
                        t_total+=el.total_days
                    })
                    t_fin_inc_date_arr.push({
                        'date':fin_inc_date_arr[fin_inc_date_arr.length-1].date,
                        'inc_dates':fin_inc_date_arr[fin_inc_date_arr.length-1].inc_dates,
                        'total_days':t_total,
                        'short_name':fin_inc_date_arr[fin_inc_date_arr.length-1].short_name
                    })
                    var data2 = {
                        key:'b9e1f8a0553623f1:639a3e:17f68ea536b',
                        ref_no:row.ref_no,
                        ...
                        dwpay:dwpay_days,
                        dwopay:dwopay_days,
                        n_dwopay:row.days_without_pay,
                        n_dwpay:row.days_with_pay,
                        leave_name:row.short_name,
                        range_dates_arr:range_dates_arr,
                        date_arr:t_fin_inc_date_arr,
                        date_filed:moment(row.created_at).format('YYYY-MM-DD'),
                        leave_type_details:row.details_of_leave_id,
                        employee_id:row.employee_id,
                        emp_status:row.empstatus,
                        used_sl:row.used_sl,
                        borrowed_vl:row.borrowed_vl,
                        bal_before_process:row.bal_before_process,
                        bal_after_process:row.bal_after_process,
                        vl_before_review:row.vl_before_review,
                        vl_after_review:row.vl_after_review,
                        sl_before_review:row.sl_before_review,
                        sl_after_review:row.sl_after_review,
                        credits_vl_val:row.credits_vl_val,
                        api_url:api_url
                    }
                }else{
                    var data2 = {
        ...
        var t_arr_dates = [];
        if(row.inclusive_dates !== 'null'){
            JSON.parse(row.inclusive_dates).forEach(el=>{
                t_arr_dates.push({
        ...
        let wpay_arr = [];
        let wopay_arr = [];
        if(row.days_without_pay !== 0){
            //get only with pay dates
            let result = JSON.parse(row.inclusive_dates).filter(o1 => !JSON.parse(row.inclusive_dates_without_pay).some(o2 => o1.date === o2.date && o1.period === o2.period));
            ...
            // wpay_arr = result;
            JSON.parse(row.inclusive_dates_without_pay).forEach(el=>{
                wopay_arr.push({
                    date:moment(el.date).format('YYYY-MM-DD'),
                    period:el.period
                })
        ...
        var data = {
            ...
            inclusive_dates_arr:t_arr_dates,
            bal_after_review:row.bal_after_review,
            bal_before_review:row.bal_before_review,
            dwpay:wpay_arr,
            dwopay:wopay_arr,
            leave_name:row.short_name
        }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                postApprovedApplicLeaveAPI({data:data2})
                .then(res=>{
                ...
                .then(res=>{
                    if(res.data.status===200){
                        var data2 = {
                            ...
                            employee_id:row.employee_id,
                            inclusive_dates:row.inclusive_dates_text,
                            inclusive_dates_arr:JSON.parse(row.inclusive_dates),
                            emp_no:row.emp_no,
                            cto_details:row.cto_details
	...
	const filteredItems = leaveApplicationData.filter(
		item => item.fullname && (item.fullname.toLowerCase().includes(filterTextReview.toLowerCase()) || item.officeCode.toLowerCase().includes(filterTextReview.toLowerCase())),
	);
    ...
    const handleDisapprovalText = (e)=>{
        setDisapproval(e.target.value)
    }
    const [filterDept,setFilterDept] = useState('')
    const [filterEmpStatus,setFilterEmpStatus] = useState('')
    const handleFilterChangeOffice = (val) =>{
        let filter;
        if(filterEmpStatus){
            filter = defaultLeaveApplicationData.filter(el=>el.empstatus === filterEmpStatus && el.officeCode === val.target.value)
        }else{
            filter = defaultLeaveApplicationData.filter(el=>el.officeCode === val.target.value)
        }
        setLeaveApplicationData(filter)
        setPage(0)
        setFilterDept(val.target.value)
    }
    const handleFilterChangeEmpStatus = (val) =>{
        let filter;
        if(filterDept){
            filter = defaultLeaveApplicationData.filter(el=>el.officeCode === filterDept && el.empstatus === val.target.value)
        }else{
            filter = defaultLeaveApplicationData.filter(el=>el.empstatus === val.target.value)
        }
        setLeaveApplicationData(filter)
        setPage(0)
        setFilterEmpStatus(val.target.value)
    }
    const handleSelect = (item) => {
        let temp = [...leaveApplicationData];
        let index = temp.findIndex(el=>el.leave_application_id === item.leave_application_id);
        temp[index].selected = !temp[index].selected;
        setLeaveApplicationData(temp)
    }
    const handleBatchApproved = async ()=>{
        let selected = leaveApplicationData.filter(el=>el.selected);
    ...
    return(
        <Box sx={{margin:'0 10px 10px 10px',paddingBottom:'20px'}}  >
            ...
            <Fade in={!isLoading}>
                ...
                <ModuleHeaderText title='Leave Application Approval'/>
            </Grid>
            </Fade>
            } */}
        {isLoading
            ...
            <Grid container sx={{mt:1}}>
                <Grid item xs={12} sm={12} md={12} lg={12} sx={{marginBottom:'10px'}}>
                    ...
                    <Box sx={{display:'flex',gap:1}}>
                        ...
                        <InputLabel id="select-filter-dept">Office/Department</InputLabel>
                        <Select
                            labelId="select-filter-dept"
                            id="select-filter-dept"
                            value={filterDept}
                            label="Office/Department"
                            onChange={handleFilterChangeOffice}
                            size='small'
                        >
                            {
                                deptNames.map((item)=>{
                        ...
                        <FormControl sx = {{width:matches?'100%':'250px'}}>
                        <InputLabel id="select-filter">Emp. Status</InputLabel>
                        <Select
                            labelId="select-filter"
                            id="select-filter"
                            value={filterEmpStatus}
                            label="Emp. Status"
                            onChange={handleFilterChangeEmpStatus}
                            size='small'
                        >
                            {
                                empStatus.map((item)=>{
                    ...
                    <IconButton color ='primary' className='custom-iconbutton' onClick = {refreshData}><CachedIcon/></IconButton>
                        ...
                        <Table>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <StyledTableCell>Inlcusive Date/s</StyledTableCell>
                                    <StyledTableCell>No. of days/hours applied</StyledTableCell>
                                    {/* <StyledTableCell>Form</StyledTableCell> */}
                                    {/* <StyledTableCell align='center'>Select <br/>
                                    <IconButton color='success' className='custom-iconbutton' sx={{background:'#fff','&:hover':{background:'#fff'}}} onClick={handleBatchApproved}><ThumbUpIcon/></IconButton>
                                    </StyledTableCell> */}
                                ...
                                
                                {
                                    ...
                                    filteredItems.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,key)=>
                                        <TableRow hover key={key}>
                                            ...
                                            <StyledTableCell>{row.officeCode}</StyledTableCell>
                                            ...
                                            {/* <StyledTableCell><Tooltip title='View Leave application form'><IconButton onClick = {()=>showLeaveForm(row)} className='custom-iconbutton' color='primary'> <VisibilityOutlinedIcon/></IconButton></Tooltip></StyledTableCell> */}
                                            {/* <StyledTableCell>
                    ...
                    ?
                        ...
                        <Grid item xs={12} sm={12} md={12} lg={12} sx={{margin:matches?'20px':'auto'}}>
                            ...
                            ?
                                ...
                                <br/>
                                <br/>
                                <TextField label = "Disapproved due to" fullWidth value = {disapproval} onChange = {handleDisapprovalText}/>
                                </>
                        ...
                        <Grid item xs={12} sm={12} md={12} lg={12}>
                            ...
                            <PreviewCTOApplicationForm auth_info = {authInfo} info={employeeInfo} coc = {employeeInfo.coc_bal} CTOHours = {employeeInfo.days_hours_applied} cto_dates = {employeeInfo.inclusive_dates_text} approval = {1} disapproval = {disapproval} date_of_filing ={employeeInfo.date_of_filing} cto_info ={ctoInfo} pendinginfo = {employeeInfo} office_head={officeHead}/>
                            <Grid item xs={12} sm={12} md={12} lg={12}>
                                <Box sx = {{display:'flex',flexDirection:'row',margin:'0 17px 10px 0',gap:1}}>
                                    {
                                        JSON.parse(employeeInfo.file_ids).map((row,key)=>
                                                <Button key = {key} variant='outlined' onClick = {()=>viewCOCFile(row)} startIcon={<AttachFileIcon/>}>View COC File Attachement #{key+1}</Button>
...
function FilterComponent(props){
    ...
    return (
        <Grid item xs={12} sx={{display:'flex',justifyContent:'flex-end'}}>
            <TextField label='Search table' placeholder='Firstname | Lastname' InputProps={{
            endAdornment: (
