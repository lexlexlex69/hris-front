...
import { getLeaveDetails,addLeaveApplication,cancelLeaveApplication,refreshData,getMonetizationInfo,getCTOAlreadyAppliedHours,getWorkSchedule, getRequestedEarnedLeaveInfo, getHolidays, getAllHolidays, getCurrentMonthCOC, getRemainingPaternityCredits, computeLeaveDays } from './LeaveApplicationRequest';
...
export default function LeaveApplicationForm(props){
            ...
            if(res.data.length !==0){
                ...
                if(has_current_year){
                    ...
                    */
                    ...
                    let finalWithPay = momentBusinessDays(moment(currDate).format('YYYY-MM-DD'), 'YYYY-MM-DD').businessSubtract(6+toAdd)._d
                    ...
                    res.data.new.forEach(el=>{
                        JSON.parse(el.working_days).forEach(el2=>{
                            ...
                            */
                            console.log(am_pm_total/60)
                            var arr_hours_val = t_arr_hours_eq.filter((val)=>{
                                return val.hours === Math.abs(am_pm_total/60);
                            })
                            t_working_days.push({
                                year:el.year,
                                day:el2.day,
                    ...
                    toast.error('No Work schedule found in this Year, please contact your AO')
                }
                
            }else{
                toast.error('No Work schedule found, please contact your AO')
            }
    ...
    const handleSetTypeOfLeave = (value) =>{
        ...
        balanceData.forEach(element => {
            ...
            switch(value.target.value){
                ...
                case 14:
                    ...
                    break;
                    case 6:
                    bal = props.availableSPL-props.onprocessSPL;
                    break;
                    case 7:
                    setapplicableDays(props.availableSoloParent)
                    bal = props.availableSoloParent
                    break;
                }
            });
        /**
        ...
        .then((response)=>{
            ...
            if(value.target.value === 6){
                ...
                setapplicableDays(props.availableSPL-props.onprocessSPL)
                // var bal = data.applicable_days;
                var bal2 = props.availableSPL-props.onprocessSPL;
                var result = [];
    ...
    const handleSortInclusiveDates = async () => {
        var sorted = selectedInclusiveDates.sort()
        var sortedTemp = selectedInclusiveDates.sort();
        ...
        if(leaveType === 3){
            ...
            // var t_dates = selectedInclusiveDates.map(el=>
            //     moment(el.toDate()).format('YYYY-MM-DD')
            // )
            // const comp = await computeLeaveDays({dates:t_dates,type:3})
             ...
             */
            // console.log(slRangeDatesWithPay);
            console.log(slRangeDatesWithPay)
            selectedInclusiveDates.forEach(el=>{
                // var hol = holidays.filter(el2=> moment(el.toDate()).format('YYYY-MM-DD')>=moment(el2.holiday_date1,'YYYY-MM-DD').format('YYYY-MM-DD') && moment(el.toDate()).format('YYYY-MM-DD')<=moment(el.holiday_date2,'YYYY-MM-DD').format('YYYY-MM-DD'));
                //check if date is between slrangedateswithpay;
                // var is_between = moment(el.toDate()).format('YYYY-MM-DD') >= moment(slRangeDatesWithPay[0]).format('YYYY-MM-DD') && moment(el.toDate()).format('YYYY-MM-DD') >= moment(slRangeDatesWithPay[slRangeDatesWithPay.length-1]).format('YYYY-MM-DD')

                if(slRangeDatesWithPay.includes(moment(el.toDate(),'MM-DD-YYYY').format('MM-DD-YYYY')) || moment(el.toDate()).isAfter(moment())){
                    isWithPay = true;
                    /**
                ...
                // }
                    ...
                    //check if continous
                    // var s_date = new Date(moment(el.toDate()).format('YYYY-MM-DD'));
            ...
            if(t_dates_not_covered.length !== 0 && !isWithPay){
                ...
                */
                ...
                // console.log(t_sl_period_days) 8
            ...
            var temp2 = [];
                ...
                //check if has late filing
                let t_late = t_daysWOP.filter(el=>el.late_filing);
                let t_late_arr = [];
                if(t_late.length>0){
                    t_late.forEach(el=>{
                        temp2.push(moment(el.date,'MM-DD-YYYY')._d)
                        temp.push(temp2)
                        temp2 = []
                    })
                }else{
                ...
                switch(leaveType){
                    ...
                    case 3:
                        ...
                        if(selectedInclusiveDates.length ===0){
                             ...
                             */
                            var has_attachment = slAttachment || (usedSL+slTotalWithoutPay+borrowedVL)>5 ?true:false;
                            var sorted = selectedInclusiveDates.sort();
                            if(slAttachment || (usedSL+slTotalWithoutPay+borrowedVL)>5){
                                if(slFile.length === 0){
                                    Swal.fire({
                                        icon:'warning',
                                        title:'Oops...',
                                ...
                                if(totalSLPeriodDays <= 1 || !SLHalfDays){
                                    ...
                                    if(totalSLPeriodDays+slAutoWithoutPay+slTotalWithoutPay<1){
                                        ...
                                        */
                                        ...
                                        if(SLPeriodDays.length !==0 && SLPeriodDays.length !== 1){
                                            if(SLPeriodDays[0].period === 'AM'){
                                                if(SLPeriodDays[1].period !== 'PM'){
                                                    Swal.fire({
                                                        icon:'warning',
                        ...
                        if(selectedInclusiveDates.length ===0){
                            ...
                            if(singleFile.length === 0){
                                ...
                                var data = {
                                    ...
                                    vl_bal:availableVL,
                                    sl_bal:availableSL,
                                    solo_before:applicableDays,
                                    solo_after:applicableDays-selectedInclusiveDates.length,
    ...
    const showLeaveDetailsApplication = ()=>{
        switch(leaveType){
            ...
            <>
            {
                ...
                null
                :
                    leaveType === 2 && balance<10
                    ?
                    <Grid item xs={12}>
                        <Typography sx={{textAlign:'center',color:red[800],fontWeight:'bold'}}>Please used VL <br/><small><em>(Cannot file FL if your balance is less than 10)</em></small></Typography>
                    </Grid>
                :
                <>
                <Grid item xs={12} sm={12} md={12} lg={12}>
                ...
                <Grid item xs={12} sm={12} md={12} lg={12}>
                    ...
                    <DatePicker
                        ...
                        //     return (
                        //     <button onClick={openCalendar} className = "custom-inclusive-dates" style={{display:'flex',alignItems:'center',overflow:'clip'}}>
                        //         {value}
                        //         <CalendarMonthIcon color='info' sx={{position:'absolute',right:30,zIndex:1,background:'#fff'}}/>
            ...
            case 3:
                ...
                <Grid item xs={12} sm={12} md={12} lg={12} sx={{mt:1}}>
                    ...
                    <DatePicker
                        ...
                        // render={(value, openCalendar) => {
                        //     return (
                        //     <button onClick={openCalendar} className = "custom-inclusive-dates" style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                        //         {value}
            ...
            case 7:
                    ...
                    <DatePicker
                        ...
                        mapDays={({ date }) => {
                            ...
                            if (isWeekend) props.className = "highlight highlight-red"
                            if(isDisabled) props.disabled=true
                            if(selectedInclusiveDates.length>=balance) props.disabled=true
                            let isHoliday = false;
                            let holiday_desc;
                            holidays.forEach(hol=>{
                                let date_1 = moment(new Date(hol.holiday_date1)).format('YYYY-MM-DD');
    ...
    const formatSubtractCreditAvailableDecimal = (leaveBalance,onProcess) => {
        if(leaveBalance === 0 || leaveBalance === 5){
            return 0;
        }else{
            ...
            <Box>
                <Box sx={{padding:'20px 5px 20px 5px'}}>
                        <Grid container spacing={2}>
                                ...
                                <Grid item xs={12} sm={12} md={12} lg={12} >
                                    ...
                                    {leaveType === 3
                                        ...
                                        :
                                            ...
                                            ?
                                            <TextField label='Specify *' sx={{marginTop:'15px'}} value = {specifyDetails} onChange={handleSetSpecifyDetails} color={specifyDetails.length ===0 ?'error':'primary'} InputProps={{inputProps:{maxLength:40}}} helperText="Max of 40 characters" placeholder='Indicate Place'/>
                            ...
                            {
                                leaveType === 1 || leaveType === 2 || leaveType === 3 || leaveType === 6  || leaveType === 7 || leaveType === 14 || leaveType === 22
                                ?
                                ...
                                ?   
                                    ...
                                    Available Credit Balance: <strong>{balance}</strong></span>
                                            ...
                                            leaveType === 6
                                            ?
                                            <span>Available Credit Balance: <strong>{balance}</strong> Day/s </span>
                                            :
                                            leaveType === 7
                                            ?
                                            <span>Available Credit Balance: <strong>{balance}</strong> Day/s </span>
                        ...
                        {leaveType
                            ...
                            <Tooltip title ='Save Leave Application'><Button sx={{'&:hover':{color:'white',background:green[800]}}} variant='contained' color="success" className='custom-roundbutton' onClick = {submitApplication} disabled = {
                                leaveType === 4
                                ?
                                    selectedInclusiveMaternityDates.length  === 0 || singleFile.length === 0
                                    ?
                                    true
                                    :
                                    false
                                ...
                                leaveType === 5
                                ?
                                    selectedInclusiveDates.length === 0 || multipleFileUpload.length === 0
                                    ?
                                    true
                                    :
                                    false
                                ...
                                ?
                                    ...
                                    false
                                :
                                    previewInclusiveDates.length === 0 || selectedInclusiveDates.length  === 0
                                    ?
                                    true
                                    :
                                    leaveType === 5
                ...
                <LargeModal open={previewModal} close={()=> setPreviewModal(false)} title='Previewing Leave Application'>
                    <Box sx={{maxHeight:'60vh',overflow:'auto',mt:2,mb:2}}>
                        {
                        leaveType === 14
                        ?
                        <div>
