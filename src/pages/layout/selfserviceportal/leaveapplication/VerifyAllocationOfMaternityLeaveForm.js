...
import {Container,Grid, Typography,Box,Button, Tooltip} from '@mui/material';
import moment from 'moment';
import { Translate } from '@mui/icons-material';
import AttachmentIcon from '@mui/icons-material/Attachment';
import ThumbUpIcon from '@mui/icons-material/ThumbUp';
import ThumbDownIcon from '@mui/icons-material/ThumbDown';
import { handleViewFile } from '../../pds/customFunctions/CustomFunctions';
import { viewFileAPI } from '../../../../viewfile/ViewFileRequest';
import { verifyExtendedMaternityInfo } from './LeaveApplicationRequest';
import Swal from 'sweetalert2';
export const VerifyAllocationOfMaternityLeaveForm = React.forwardRef((props,ref)=>{
    const [hasViewFile,setHasViewFile] = useState(true)
    const handleViewFile = (id)=>{
        viewFileAPI(id,'')
        setHasViewFile(false)
    }
    const handleApproved = ()=>{
        console.log(props.info)
         Swal.fire({
            icon:'info',
            title:'Approving of notice of allocation of maternity leave',
            html:'Please wait...',
            timer:1500,
        ...
        Swal.showLoading()
        var data = {
            allocation_id:props.info.allocation_id,
            allocation_number_days:props.info.allocation_number_days,
            benefit_emp_no:props.info.benefit_emp_no
        }
        verifyExtendedMaternityInfo(data)
        .then(res=>{
            ...
            <Grid container>
                ...
                <Grid container sx = {{marginTop:'10px',display:'flex',flexDirection:'row' ,border:'solid 1px'}}>
                    ...
                    <Grid item xs = {7} sx={{height:'35px',borderRight:'solid 1px',borderBottom:'solid 1px'}}>
                            ...
                            <Typography sx={{fontSize:'10px', padding:'5px 0 0 5px',fontWeight:'600'}}>{props.info.employee_contact_details}</Typography>
                    ...
                    <Grid item xs = {12} sx={{padding:'10px'}}>
                        <Box sx={{display:'flex',flexDirection:'row',justifyContent:'space-evenly'}}>
                            ...
                            <strong>____________<u>{props.info.fname} {props.info.mname?props.info.mname.charAt(0)+'.':' '} {props.info.lname}</u>__________</strong><br/>
                    ...
                    <Grid item xs = {5} sx={{borderBottom:'solid 1px',borderRight:'solid 1px'}}>
                        <Box sx={{display:'flex',flexDirection:'column'}}>
                            ...
                            <Typography sx={{fontSize:'10px', padding:'5px 10px 0 10px'}}>
                                <input className='ex' type = "checkbox" checked = {props.info.benefit_relationship === "Childs father" ? true:false}/> Child’s father
                            </Typography>
                            <Typography sx={{fontSize:'10px', paddingLeft:'10px'}}>
                                <input type = "checkbox" checked = {props.info.benefit_relationship === 'Alternate caregiver' ? true:false}/> Alternate caregiver
                            </Typography>
                            <Typography sx={{fontSize:'10px', paddingLeft:'24px'}}>
                                <input type = "checkbox" checked = {props.info.benefit_relationship === 'Alternate caregiver' ?props.info.benefit_relationship_details === "Relative within fourth degree of consanguinity" ? true:false:false}/> Relative within fourth degree of consanguinity<br/>
                            ...
                            <Typography sx={{fontSize:'10px', paddingLeft:'24px'}}>
                                <input type = "checkbox" checked = {props.info.benefit_relationship === 'Alternate caregiver' ?props.info.benefit_relationship === "Current partner sharing the same household" ? true:false :false}/> Current partner sharing the same household
                            </Typography>
                            </Grid>
                        </Box>
    ...
    height: '50px',top:'-15px',left:'50px'}}/>
                ...
                <Grid container sx = {{marginTop:'15px',display:'flex',flexDirection:'row' ,border:'solid 1px'}}>
                    ...
                    <Grid item xs = {3} sx={{borderRight:'solid 1px',padding:'10px 0 0 0',height:'60px'}}>
                            <Typography sx={{fontSize:'10px', paddingLeft:'10px'}}>
                                <input type = "checkbox" checked = {props.info.benefit_proof_relationship === "Childs Birth Certificate" ? true:false}/> Child’s Birth Certificate
                            </Typography> 
                    </Grid>
                    <Grid item xs = {3} sx={{borderRight:'solid 1px',padding:'10px 0 0 0',height:'60px'}}>
                            <Typography sx={{fontSize:'10px', paddingLeft:'10px'}}>
                                <input type = "checkbox" checked = {props.info.benefit_proof_relationship === "Marriage Certificate" ? true:false}/> Marriage Certificate
                            </Typography> 
                    </Grid>
                    <Grid item xs = {3} sx={{borderRight:'solid 1px',padding:'10px 0 0 0',height:'60px'}}>
                            <Typography sx={{fontSize:'10px', paddingLeft:'10px'}}>
                                <input type = "checkbox" checked = {props.info.benefit_proof_relationship === "Barangay Certificate" ? true:false}/> Barangay Certificate
                            </Typography> 
                    </Grid>
                    <Grid item xs = {3} sx={{height:'60px',padding:'10px 0 0 0'}}>
                            <Typography sx={{fontSize:'10px', paddingLeft:'10px'}}>
                                <input type = "checkbox" checked = {props.info.benefit_proof_relationship === "Other bona fide document/s that can prove filial relationship" ? true:false}/> Other bona fide document/s that can prove filial relationship
                            </Typography> 
                    </Grid>
                </Grid>
                <Grid item xs = {12} sx={{marginTop:'15px', borderTop:'solid 2px'}}>
                ...
                <Grid item xs={12} sx={{mt:1,display:'flex',justifyContent:'flex-end'}}>
                    <Tooltip title='View proof of relationship'><Button variant='outlined' onClick={()=>handleViewFile(props.info.benefit_relationship_proof_fileid)}><AttachmentIcon/></Button></Tooltip>
                    
                </Grid>
                <Grid item xs={12} sx={{mt:1,display:'flex',justifyContent:'flex-end'}}>
                    <Tooltip title='Approved'><Button variant='outlined' color='success' disabled={hasViewFile} onClick={handleApproved}><ThumbUpIcon/></Button></Tooltip>
                    &nbsp;
                    <Tooltip title='Dissapproved'><Button variant='outlined' color='error' disabled={hasViewFile}><ThumbDownIcon/></Button></Tooltip>
                </Grid>
            </Grid>
        </Container>
        </div>
...
export default VerifyAllocationOfMaternityLeaveForm
