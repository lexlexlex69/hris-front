...
export default function RequestedReschedule(props,ref){
    // media query
        ...
        <Box sx={{margin:matches?0:'0 20px 0 20px',mt:1,mb:1}}>
            <Grid container>
                <Grid item xs={12}>
                    <Paper sx={{p:1}}>
                    <Box>
                        <CardHeaderText color={red[500]} title='Requested Reschedule'/>
                    </Box>
                            ...
                            <tbody>
                               {
                                props.data.slice((page-1)*rowsPerPage,(page-1)*rowsPerPage+rowsPerPage).map((item,key)=>
                                    ...
                                    <tr className="hover-row">
                                        ...
                                        <td style={{textAlign:'left'}}>
                                            {props.formatReschedDetails(item.details)}
                                        </td>
                                        <td style={{textAlign:'left'}}>
                                            {item.reason}
                                        </td>
                                        ...
                                        <td style={{textAlign:'right'}}>
                                            <Tooltip title = 'Delete request'><span><IconButton color='error' disabled={item.status==='FOR REVIEW'?false:true} onClick={()=>props.handleDeleteMainReschedule(item,key)} className='custom-iconbutton' sx={{'&:hover':{color:'#fff',background:red[800]}}} size="small"><DeleteIcon/></IconButton></span></Tooltip>
                                        </td>
