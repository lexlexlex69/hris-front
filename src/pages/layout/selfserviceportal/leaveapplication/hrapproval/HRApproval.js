...
import { approvedHRReview, getHRForApproval, submitHRApprovalForUpdatingReview, submitHRApprovalForUpdatingReviewAPI, submitLeaveApplicationUpdateCTO, submitLeaveBackToReview, submitLeaveDisapproval, submitLeaveReconciliation } from './HRApprovalRequest';
...
export default function HRApproval(){
    ...
    const submitReviewApplication = (event) =>{
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if(recommendation === 'Certified Correct'){
                ...
                }else if (recommendation === 'For Updating'){
                    ...
                    var t_data_leave = {
                        data:employeeInfo,
                        api_url:api_url+'/submitHRApprovalForUpdatingReviewAPI',
                    }
                    submitHRApprovalForUpdatingReviewAPI(t_data_leave)
                    .then(res=>{
                        console.log(res.data)
                        if(res.data.status === 200){
                            var t_data;
                        ...
                        if (typeof err.response === 'undefined') {
                            alert('A network error occurred. '
                                + 'This could be a CORS issue or a dropped internet connection. A new window will open, please proceed')
                        }
                        window.open(api_url)
                    })
                    ...
                    try{
                        let t_data2 = {
                            id:employeeInfo.leave_application_id,
                            incharge_id:employeeInfo.incharge_id,
                            dept_code:employeeInfo.dept_code,
                            remarks:remarks
                        }
                        const res = await submitLeaveDisapproval(t_data2);
                        if(res.data.status === 200){
                ...
                }else if(recommendation === 'For Review'){
                    try{
                        let t_data2 = {
                            id:employeeInfo.leave_application_id,
                            incharge_id:employeeInfo.incharge_id,
                        ...
                        const res = await submitLeaveBackToReview(t_data2);
                        if(res.data.status === 200){
                            APISuccess(res.data.message)
                            setmodalOpen(false)
                            setRecommendation('')
                    ...
                    try{
                        ...
                        const res = await submitLeaveReconciliation(t_data2);
                        if(res.data.status === 200){
                            APISuccess(res.data.message)
                            setmodalOpen(false)
                            setRecommendation('')
    ...
    useEffect(()=>{
        if(recommendation === 'For Updating'){
            // var top = document.getElementById('for-updating-div').offsetTop;
            // window.scrollTo(0, top);
            var elmntToView = document.getElementById("for-updating-div");
            elmntToView.scrollIntoView(); 
        }
        if(recommendation === 'For Reconciliation'){
            // var top = document.getElementById('for-updating-div').offsetTop;
            // window.scrollTo(0, top);
            var elmntToView = document.getElementById("for-reconciliation-div");
            elmntToView.scrollIntoView(); 
        }
        if(recommendation === 'Disapproved'){
            // var top = document.getElementById('for-updating-div').offsetTop;
            // window.scrollTo(0, top);
            var elmntToView = document.getElementById("for-disapproval-div");
            elmntToView.scrollIntoView(); 
        }
    },[recommendation])
    ...
    const forDisapproval = () => {
        return (
            <Grid item xs={12} id = 'for-disapproval-div'>
                <TextField label = 'Remarks' multiline minRows={2} fullWidth value = {remarks} onChange={(val)=>setRemarks(val.target.value)}/>
            </Grid>
        )
    }
    const forReconciliation = () => {
        return (
            <Grid item xs={12} id = 'for-reconciliation-div'>
                <TextField label = 'Remarks' multiline minRows={2} fullWidth value = {remarks} onChange={(val)=>setRemarks(val.target.value)}/>
            </Grid>
        )
    }
    const forUpdating = () =>{
        return (
            <Grid item xs={12} id = 'for-updating-div'>
                {
                ...
                </Box>
                {
                    ...
                    ?
                    <Grid container sx={{mt:2}} id = 'for-updating-div'>
                        <Grid item xs={12}>
    ...
    const handleUpdateCTO = (item) =>{
        ...
        postUpdatingReviewApplicLeaveAPI(t_data_leave)
        .then(res=>{
            console.log(res.data)
            // Swal.close();
            if(res.data.status === 200){
            ...
            <Grid  item xs={12} sm={12} md={12} lg={12}>
                ...
                {  
                        ...
                        <Table>
                            ...
                            <TableBody>
                                
                                {
                                    ...
                                    ?
                                    <TableRow><StyledTableCell colSpan={9} align='center'>No Data</StyledTableCell></TableRow>
                                    :
                                    filteredItems.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,key)=>
                                        <TableRow hover key={key}>
                                            ...
                                            <StyledTableCell>
                                            {/* <Tooltip title='Review application'><IconButton color='success' sx={{'&:hover':{color:'#fff',background:green[800]}}} size={matches?'small':'large'} onClick={()=>setInfo(row)} className='custom-iconbutton'><RateReviewIcon/></IconButton> */}
                                            <Tooltip title='Review application'>
                                            <Button color='success' variant='contained' size='small' onClick={()=>setInfo(row)} className='custom-roundbutton' startIcon={<RateReviewIcon/>}>Review
                                            </Button>
                                            </Tooltip>
                                            </StyledTableCell>
                                        </TableRow>
                    ...
                    <LargeModal open={modalOpen} close={()=> setmodalOpen(false)} title='Reviewing Leave Application'>
                        <form onSubmit={submitReviewApplication} id = 'for-updating-div'>
                            <Box sx={{m:1,maxHeight:matches?'50vh':'70vh',overflow:'auto'}}>
                            <Grid container>
                            <div style={{position:'relative'}}>
                            {
                                    ...
                                    {employeeInfo.leave_type_id === 1 || employeeInfo.leave_type_id === 2 || employeeInfo.leave_type_id === 4
                                        ...
                                        <FormControl fullWidth>
                                            ...
                                            >
                                                <MenuItem value={'Certified Correct'}>Certified Correct</MenuItem>
                                                <MenuItem value={'For Updating'}>For Updating</MenuItem>
                                                <MenuItem value={'For Review'}>Return to AO</MenuItem>
                                                <MenuItem value={'For Reconciliation'}>For Reconciliation</MenuItem>
                                ...
                                :
                                    ...
                                    <Grid item xs={12} sm={12} md={12} lg={12}  id ='request-action'>
                                        <FormControl fullWidth>
                                            <InputLabel id="demo-simple-select-label">Action</InputLabel>
                                            <Select
                                                labelId="demo-simple-select-label"
                    ...
                    <LargeModal open={modalUpdatingOpen} close = {()=> setmodalUpdatingOpen(false)} title = 'Updating LEAVE APPLICATION Info'>
