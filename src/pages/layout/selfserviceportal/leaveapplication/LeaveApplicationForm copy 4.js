...
import { getLeaveDetails,addLeaveApplication,cancelLeaveApplication,refreshData,getMonetizationInfo,getCTOAlreadyAppliedHours,getWorkSchedule, getRequestedEarnedLeaveInfo, getHolidays, getAllHolidays } from './LeaveApplicationRequest';
...
export default function LeaveApplicationForm(props){
    ...
    useEffect(()=>{
        console.log(props.presentDaysData)
    ...
    const handleSortInclusiveDates = () => {
        ...
        if(leaveType === 3){
            ...
            var i = len-1;
            // console.log(alreadyAppliedDays)
            ...
            selectedInclusiveDates.forEach(el=>{
                ...
                if(t_period.length >0){
                    t_total_has_period+=.5;
                }
            })
            var totalWithPay=0;
            console.log(daysWOP);
            if(daysWOP.length === 0 ){
                if((selectedInclusiveDates.length-t_total_has_period) > balance){
                    totalWithoutPay = selectedInclusiveDates.length-t_total_has_period
                    ...
                    if(balance>=0){
                        ...
                        totalWithoutPay = totalWithoutPay-t_count;
                        var t_remaining_bal = balance-t_count;
                        var t_borrow = (totalWithoutPay-t_remaining_bal).toFixed(3)
                        ...
                        if(availableVL>=.5){
                            if(availableVL>=t_borrow){
                                 ...
                                 */
                                var t_sl_vl = t_remaining_bal+availableVL;
                                /**
                                ...
                                setBorrowedVL((t_sl_vl_count-t_remaining_bal).toFixed(3))
                                setUsedSL((t_count+t_remaining_bal).toFixed(3))

                                totalWithPay = ((t_sl_vl_count-t_remaining_bal)+(t_count+t_remaining_bal)).toFixed(3)
                                ...
                                if(selectedInclusiveDates.length ===0){
                                    ...
                                    if(commutation.length === 0){
                                        ...
                                        var data = {
                                            ...
                                            days_with_pay:selectedInclusiveDates.length>Math.floor(balance) ? balance>=.5?vlWpay: Math.floor(balance):selectedInclusiveDates.length,
                                            // days_with_pay:vlWpay,
                                            days_without_pay:selectedInclusiveDates.length>Math.floor(balance) ? balance>=.5?selectedInclusiveDates.length-vlWpay:selectedInclusiveDates.length - Math.floor(balance):0,
                                        ...
                                        if(is_continuous){
                                            if(specifyDetails.length !==0){
                                                var data = {
                                                    ...
                                                    sl_file:slFile,
                                                    borrowed_vl:borrowedVL,
                                                    used_sl:usedSL === 0 ?totalSLPeriodDays:usedSL,
                                                    vl_bal:availableVL,
                                                    is_late_filing:isLateFiling
    ...
    useEffect(()=>{
        // console.log(daysOfMonetization)
        if(parseInt(daysOfMonetization) ===0){
            settotalVL(0)
            settotalSL(0)
        }else{
    ...
    const checkMonetizationAboveHalf = () => {
        ...
        if(parseInt(daysOfMonetization)>=total_sl_vl){
            return(
                <Grid item xs={12} sm={12} md={12} lg={12}>
                    <Tooltip title="Please upload a PDF/Image File" placement='top'>
                    <TextField type = "file" label="Letter of Request *" fullWidth InputLabelProps={{shrink:true}} onChange = {handleSingleFile} InputProps={{ inputProps: { accept:'image/*    ,.pdf'}}}/>
    ...
    const monetizationVLFormat = () => {
        ...
        if(availableVL<=5){
            ...
            return (availableVL-5).toFixed(0)
        }
    }
        ...
        switch(leaveType){
            ...
            case 3:
                ...
                <Grid item xs={12} sm={12} md={12} lg={12} sx={{mt:1}}>
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={12}>
                        <TextField label='No. of Days Applied' fullWidth  value = {totalSLPeriodDays+slTotalWithoutPay} InputLabelProps={{ shrink: true }} readOnly />
                    </Grid>
                    {
                    ...
                    :
                        ...
                        isApplicableForFiling === true
                            ...
                            <>
                                ...
                                <Grid item xs={12} sm={12} md={12} lg={12}>
                                    <Tooltip title="Please upload a PDF/Image File" placement='top'>
                                    <TextField type = "file" label="Proof of child's delivery / Marriage Certificate *" fullWidth InputLabelProps={{shrink:true}} onChange = {handleSingleFile} InputProps={{ inputProps: { accept:'image/*    ,.pdf'} }} disabled={selectedInclusiveDates.length !== 0 ?false:true}/>
                                    </Tooltip>
                                </Grid>
                                </>
                            :
    ...
    const handleVLWOPAY = (bal)=>{
        ...
        // console.log(t_count)
        if(bal>.5){
            return selectedInclusiveDates.length-t_count
        }else{
        ...
        <>
        {
            ...
            <Box>
                        ...
                        <Grid container spacing={2}>
                            ...
                            
                            {
                                ...
                                ?   
                                    ...
                                    ?
                                    <span> 
                                    Available Balance: <strong>{parseFloat(balance).toFixed(3)} Hour/s </strong></span>
                                    :
                                        leaveType === 3
                                        ?
                                        <span> 
                                    Available Balance: <strong>{balance}</strong> Day/s</span>
                                        :
                                    ...
                                    Available Balance: <strong>{balance}</strong> Day/s </span>
                                            ...
                                            leaveType === 22
                                            ?
                                            <span>Available Balance: <strong>{maternityBal}</strong> Day/s </span>
                                            :
                                            ''
                                :
                                ''
                ...
                <div style={{ display: "none" }}>
                    <PreviewCTOApplicationForm ref={cocRef} info={employeeInfo} auth_info={props.authInfo} coc = {balanceData.length !==0 ? balanceData[0].coc_bal:0} CTOHours = {CTOHours} cto_dates = {ctodatestext} date_of_filing ={new Date()} cto_info = {ctoInfo} office_ao = {officeAO}/>
                </div>
                </Box>
            :
            <Stack>
