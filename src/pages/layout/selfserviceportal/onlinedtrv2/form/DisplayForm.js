...
export const DisplayForm = ({data,rowsToAdd,empInfo,signatory,from,to,alreadyAppliedRectification,setAlreadyAppliedRectification,updateAppliedRectification,rectificationData,rawLogs,daysNumber,type})=>{
    ...
    const handleViewRawLogs = (item)=>{
        console.log(rawLogs)
        console.log(item)
        var logs = rawLogs.filter(el=>el.trandate === moment(item.date).format('YYYY-MM-DD'))
        if(logs.length>0){
            var sorted = logs.sort((a, b) => moment(a.timein,'HH:mm') - moment(b.timein,'HH:mm'));
    ...
    const defaultRow = (item) =>{
        let time_in_rec = item.time_in?[]:checkRectification(item.date,'Time In');
        let break_out_rec = item.break_out?[]:checkRectification(item.date,'Break Out');
        let break_in_rec = item.break_in?[]:checkRectification(item.date,'Break In');
        let time_out_rec = item.time_out?[]:checkRectification(item.date,'Time Out');
    ...
    const matchDay = (date) =>{
        ...
        if(items.length>0){
            ...
            return (
                <tr>
                <Tooltip title='View Biometric Logs' placement='top'><td className='hover-date' style={{color:'#fff',background:blue[500],cursor:'pointer',pointerEvents:moment(date).format('YYYY-MM-DD')>moment().format('YYYY-MM-DD')?'none':'auto'}} onClick={()=>handleViewRawLogs(item)}>{moment(date).format('D')}</td>
                </Tooltip>
                {
                    item.day_type === 1
                    ?
            ...
            return (
                <tr>
                    <Tooltip title='View Biometric Logs' placement='top'><td className='hover-date' style={{color:'#fff',background:blue[500],cursor:'pointer'}} onClick={()=>handleViewRawLogs({date:date})}>{moment(date).format('D')}</td>
                </Tooltip>
