import { Box,Grid,Skeleton,Fade,Paper,Typography, Button,Tooltip } from '@mui/material'
import React,{useEffect, useState} from 'react'
import { getAllForReviewPassSlip,reviewActionPassSlip } from './PassSlipUndertimeRequest'
...
export default function PassSlipReview
(){
    // media query
    ...
    const columns = [
        ...
        {
            name:'Action',
            selector:row=><Box><Tooltip title='Approved'><Button color='success' onClick = {()=>action(1,row)}><ThumbUpOutlinedIcon/></Button></Tooltip><Tooltip title='Disapproved'><Button color='error' onClick = {()=>action(0,row)}><ThumbDownOutlinedIcon/></Button></Tooltip></Box>
        },
    ]
                ...
                <Box sx={{margin:'20px'}}>
                    <Grid container>
                        <Grid item xs={12} sm={12} md={12} lg={12} sx={{margin:'10px 0 10px 0'}}>
                            ...
                            <ModuleHeaderText title='Pass Slip Review'/>
                        </Grid>
                        <Grid item xs={12}>
                            <DataTable
                                data={data}
