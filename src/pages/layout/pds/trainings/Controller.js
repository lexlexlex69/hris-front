...
export const getEmployeeTrainings = (id, setState, controller, setPageTotal, perPage, setTableData, setDefaultState, setLoader) => {
    axios.post(`/api/pds/trainings/getEmployeeTrainings`, { id: id }, { signal: controller.signal })
        .then(res => {
            setLoader(true)
            if (res.status === 200) {
                setState(res.data)
...
export const getTrainingsUpdates = (id, setState, controller, setPageTotal, perPage, setTableData, setUpdates, setDefaultState, setLoader) => {
    axios.post(`/api/pds/trainings/getTrainingsUpdates`, { id: id }, { signal: controller.signal })
        .then(res => {
            ...
            if (res.status === 200) {
                ...
                setDefaultState(res.data.trainings)
                let newArr = res.data.trainings.slice(0, perPage)
                setTableData(newArr)
                setUpdates(res.data.updates)
                setPageTotal(res.data.trainings.length)
            }
        })
        .catch(err => {
            setLoader(true)
...
export const handleUpdate = (id, record, setRecord, setState, defaultState, setTableData, perPage,setPage,setPageTotal) => {
    ...
    axios.post(`/api/pds/trainings/updateEmployeeTrainings`, {
        id: id,
        record: recordArray
    })
        .then(res => {
