...
const UpdateRecognition = (props) => {
    ...
    // file state
    const handleUpdate = async () => { // make updates 
        ...
        // filteredValuesObj.type_id = 2
        filteredValuesObj.rowId = props.data.id

        let recordToUpdate = props.recognitionRecord.map(item => item)
        recordToUpdate.push(filteredValuesObj)
        let recognitionToUpdate = props.recognition.map((item, index) => {
        ...
        props.setRecognitionRecord(recordToUpdate)
        props.setRecognition(recognitionToUpdate)
        props.handleClose()
    }
            ...
            <Box>
                <Grid container spacing={1} >
                    <Grid item xs={12} sm={12} md={12} lg={12} sx={{ mt: 1, display: 'flex', flexDirection: 'row', alignItems: 'flex-end', gap: 1, justifyContent: 'flex-start' }}>
                        <TextField fullWidth label="NON-ACADEMIC DISTINCTIONS/RECOGNITION" variant='filled' defaultValue={props.data.description} inputRef={description} />
                    </Grid>
                    <Grid item xs={12} sm={12} md={12} lg={12}>
                        <Box sx={{ mt: 3, display: 'flex', justifyContent: 'space-between' }}>
                            <Button variant="contained" color="warning" onClick={handleUpdate} startIcon={<EditIcon />} > Update</Button>
...
export default UpdateRecognition
