...
const UpdateSpecialSkills = (props) => {
    // media query
    ...
    const handleUpdateTrainings = async () => { // make updates 
        ...
        // filteredValuesObj.type_id = 1
        filteredValuesObj.rowId = props.data.id

        let recordToUpdate = props.specialSkillsRecord.map(item => item)
        recordToUpdate.push(filteredValuesObj)
        //console.log(recordToUpdate)
        let specialSkillsToUpdate = props.specialSkills.map((item, index) => {
        ...
        props.setSpecialSkillsRecord(recordToUpdate)
        props.setSpecialSkills(specialSkillsToUpdate)
        props.handleClose()
    }
            ...
            <Box>
                <Grid container spacing={1} >
                    <Grid item xs={12} sm={12} md={12} lg={12} sx={{ mt: 1, display: 'flex', flexDirection: 'row', alignItems: 'flex-end', gap: 1, justifyContent: 'flex-start' }}>
                        <TextField fullWidth label="SPECIAL SKILLS" variant='filled' defaultValue={props.data.description} inputRef={description} />
                    </Grid>
...
export default UpdateSpecialSkills
