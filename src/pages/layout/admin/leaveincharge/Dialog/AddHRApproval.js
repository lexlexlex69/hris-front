...
export default function AddHRApproval(props){
    ...
    const handleSelect = (data)=> {
        console.log(data)
        setEmpid(data.id)
        setName(`${data.fname} ${formatMiddlename(data.mname)} ${data.lname}`)
        ...
        <Box sx={{m:2}}>
            ...
            <Grid container spacing={2}>
                <Grid item xs={12} sx={{display:'flex',justifyContent:'space-between'}}>
                    <TextField label ='Employee Name' fullWidth value = {name} InputLabelProps={{shrink:true}} inputProps={{readOnly:true}} required read/>
                &nbsp;
                            ...
                            return (
                                <Box sx={{display:'flex',gap:1,alignItems:'center'}} key = {key}>
                                    ...
                                    <Autocomplete
                                        ...
                                        // disableCloseOnSelect
                                    />
                                    <IconButton className='custom-iconbutton' color='error' onClick={()=>handleDelRow(item,key)}><Delete/></IconButton>
                                </Box>
                            )
                        })
                    }
