...
import { addDTRConfig } from "../DTRMgtConfigRequests";
import { APILoading } from "../../../apiresponse/APIResponse";
export const Add = ({offices,updateData,close})=>{
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if(allOffices){
            if(data.emp_info?.lname &&data.actions?.length>0){
                ...
                var t_data = {
                    emp_id:data.emp_info.id,
                    offices:offices,
                    actions:data.actions,
                }
                const res = await addDTRConfig(t_data);
