...
import { addDivSection, getDivSecUnsetEmp } from '../../EmpManagementRequest';
import { APILoading } from '../../../../apiresponse/APIResponse';
export const AddSection = ({selectedDiv,updateData,close}) =>{
    ...
    useEffect(async ()=>{
        ...
        const res = await getDivSecUnsetEmp({dept_code:selectedDiv.dept_code,dept_div_id:selectedDiv.dept_div_id})
    ...
    const handleSubmit =  async (e)=>{
        e.preventDefault();
        try{
            APILoading('info','Adding new section','Please wait...')
            let ids = empList.filter(el=>el.selected).map(el2=>el2.emp_no);
            if(ids.length>0){
                const res = await addDivSection({dept_code:selectedDiv.dept_code,dept_div_id:selectedDiv.dept_div_id,section_name:sectionName,ids:ids})
                console.log(res.data)
    ...
    return (
        ...
        <Grid container spacing={1}>
            <Grid item xs={12}>
                <TextField label  = 'Section Name' value = {sectionName} onChange={(val)=>setSectionName(val.target.value)} fullWidth required/>
            </Grid>
            <Grid item xs={12}>
