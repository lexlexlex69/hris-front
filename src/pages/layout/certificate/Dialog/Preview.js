...
import {Grid,Typography,Table,TableHead,TableContainer,TablePagination,TableRow,TableBody,IconButton,Checkbox,Modal,TextField,Box,Button,Tooltip,FormGroup,FormControlLabel} from '@mui/material';
// media query
...
const StyledTableCell = styled(TableCell)(({ theme }) => ({
  ...
  export default function Preview(props){
    ...
    const [rowsPerPage, setRowsPerPage] = useState(5);useEffect(()=>{
        setSelectedIds(props.traineeIds)
        console.log(props.traineeIds)
    },props.traineeIds)
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12} sx={{display:'flex',justifyContent:'space-between'}}>
                <Grid item xs={6}>
                    <FormGroup row>
                        <FormControlLabel disabled control={<Checkbox defaultChecked/>} label="Completed" />
                        <FormControlLabel disabled control={<Checkbox/>} label="Incomplete" />
                    </FormGroup>
                </Grid>
                <Grid item xs={6} sx={{display:'flex',justifyContent:'flex-end'}}>
                    <Tooltip title='Download selected list'><span><IconButton onClick={downloadList} color='primary' disabled = {selectedIds.length === 0 ? true:false}><FileDownloadIcon/></IconButton></span></Tooltip>
                        ...
                        <TableHead>
                            <TableRow>
                                ...
                                <StyledTableCell>Evaluation</StyledTableCell>
                                {
                                    ...
                                    :
                                    <StyledTableCell>Select <br/><Checkbox checked = {isSelectAll} onChange = {handleSelectedAll} sx={{color:'#fff','&.Mui-checked': {color: '#fff'}}} checkedIcon={<RemoveOutlinedIcon/>}/></StyledTableCell>
                        ...
                        <TableBody>
                            {
                                ...
                                props.traineeNames.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,key)=>
                                    <TableRow key={key} hover>
                                        ...
                                        <StyledTableCell><Checkbox disabled defaultChecked={props.compRqmt.includes(row.emp_id) ? true:false}/></StyledTableCell>
                                        <StyledTableCell>
                                            <Checkbox disabled defaultChecked={props.compEvaluation.includes(row.emp_id) ? true:false}/>
                                        </StyledTableCell>
                                        <StyledTableCell><Checkbox checked={selectedIds.includes(row.emp_id) ? true:false} onChange = {()=>handleSelect(row.emp_id)}/></StyledTableCell>
                                    </TableRow>
                                )
                            }
                        </TableBody>
