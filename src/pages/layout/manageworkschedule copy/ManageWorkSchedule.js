...
import { getWorkScheduleTemplate,getOffices,searchFilter,addEmployeeWorkSched,getScheduleData,deleteScheduleData,searchDepartmentFilter,showTemplateEmployeeList, getAddedWorkSched, getRequestedDelWorkSched, getRequestedUpdateWorkSched } from './WorkScheduleRequest';
// media query
...
export default function ManageWorkSchedule(){
    ...
    useEffect(()=>{
        ...
        .then(res=>{
            ...
            setNewAddWorkScheduleData(res.data)
        }).catch(err=>{
            console.log(err)
        })
                ...
                <Fade in = {!isLoading}>
                    ...
                    <Modal
                        ...
                        <Box sx={empListStyle}>
                            ...
                            <Typography id="modal-modal-title" sx={{textAlign:'center',padding:'10px',color:'#fff',background:'#0d6efd',borderTopLeftRadius:'10px',borderTopRightRadius:'10px',margin:'-2px',textTransform:'uppercase'}} variant="h6" component="h2">
                            { showAllEmpList?'':empListModalName+"'s"} Employee List
                            </Typography>
                            <Box>
