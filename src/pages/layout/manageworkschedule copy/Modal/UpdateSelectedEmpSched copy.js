...
export default function UpdateSelectedEmployeeEmpSched(props){
    ...
    useEffect(()=>{
        setScheduleUpdateData(props.data)
    ...
    const clickDate = (value) =>{
        ...
        for(var i = 0; i<scheduleUpdateData.length;i++){
            if(moment(scheduleUpdateData[i].date).format('MM-DD-YYYY') === moment(value._d).format('MM-DD-YYYY')){
                has_sched = true;
                break;
            }
        }
    ...
    const renderDay = (props, currentDate, selectedDate) => {
        ...
        var day = moment(currentDate._d).format('DD')
