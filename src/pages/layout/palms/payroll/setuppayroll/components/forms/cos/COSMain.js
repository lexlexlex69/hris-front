...
export const COSMain = ({empList,index,loans,perPage,isGrandTotal,printData,periodFrom,periodTo,wTax}) =>{
    // useEffect(()=>{
    //     const chunkSize = 10;
    //     let arr = [];
    ...
    useEffect(()=>{
        ...
        var accrued = empList.map((el)=>{
            return (parseFloat(el.m_salary)/2)-parseFloat(el.total_wopay)
        }).reduce((total,a)=>{
            return total + a
        })
        ...
        var amount_due = empList.map((el)=>{
            return (parseFloat(el.m_salary)/2)-parseFloat(el.total_wopay)-parseFloat(el.tot_contri)
        }).reduce((total,a)=>{
            return total + a
        })
        ...
        var ph_per = empList.map((el)=>{
            return parseFloat(truncateToDecimals(parseFloat(el.ph_personal_share)))
        }).reduce((total,a) =>{
            return total + a
        })
    },[empList])
    ...
    const runningTotal = (type,index) =>{
        ...
        switch(type){
            ...
            case 'accrued':
                ...
                for(i;i <= index;i++){
                    var t_total = printData[i].map(el=>{
                        return parseFloat((el.m_salary/2)-el.total_wopay)
                    }).reduce((total,a)=>{
                        return total + a
                    })
                    total+=t_total;
                    ...
                    <tbody>
                    {
                        ...
                        return(
                            <tr key={item.id}>
                                ...
                                <td align="right">
                                    {formatWithCommas(parseFloat(item.tot_contri).toFixed(2))}
                                </td>
