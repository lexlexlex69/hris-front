...
export const AddSpecial = ({offices}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            ...
            let t_data = {
                ...
                type:selectedType,
                emp_status:selectedEmpStatus.emp_status
            }
    ...
    const printDiv = () => {
        switch(selectedType){
            ...
            return (
                ...
                <PEIPrint ref={printPayroll} empList = {data} signatories={signatories} selectedOffice = {selectedOffice} year = {selectedYear} empStatus = {selectedEmpStatus?.status} user={user} isHR = {isHR} deptHead = {deptHead}/>
            </div>);
            break;
            case 'CNA':
            return (
                <div style={{display:'none'}}>
                <CNAPrint ref={printPayroll} empList = {data} signatories={signatories} selectedOffice = {selectedOffice} year = {selectedYear} empStatus = {selectedEmpStatus?.status} user={user} isHR = {isHR} deptHead = {deptHead}/>
            </div>);
            break;
            case 'SRI':
            return (
                <div style={{display:'none'}}>
                <SRIPrint ref={printPayroll} empList = {data} signatories={signatories} selectedOffice = {selectedOffice} year = {selectedYear} empStatus = {selectedEmpStatus?.status} user={user} isHR = {isHR} deptHead = {deptHead}/>
            </div>);
            break;
            case 'YEAR-END':
            return(
                <div style={{display:'none'}}>
                    <YearEndPrint ref={printPayroll} empList = {data} signatories={signatories} selectedOffice = {selectedOffice} year = {selectedYear} empStatus = {selectedEmpStatus?.status} user={user} isHR = {isHR} deptHead = {deptHead}/>
                </div>
            )
            break;
        }
        ...
        <Grid container spacing={1}  sx={{pt:1}}>
            ...
            <form style={{width:'100%',display:'flex',flexDirection:'row',gap:1}} onSubmit={handleSubmit}>
                ...
                <Autocomplete
                    ...
                    id="combo-box-dept"
                    options={empStatus}
                    getOptionLabel={(option) => option.emp_status}
                    isOptionEqualToValue={(option, value) => option.emp_status === value.emp_status }
                    sx={{ width: 250 }}
                    // fullWidth
                    size="small"
