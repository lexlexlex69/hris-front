...
export const PayGroupDtl = ({selectedData,setPayGroupDtlData,setPayrollGroupData,selectedGroupDtl,setSelectedGroupDtl,loanType})=>{
    ...
    const filterData = data.filter(el=>el.fname.toUpperCase().includes(searchVal.toUpperCase()) || el.lname.toUpperCase().includes(searchVal.toUpperCase()));
    ...
    useEffect(()=>{
        const getData = setTimeout(() => {
            ...
            let t_deductions = parseFloat(temp?.provident)+parseFloat(temp?.pagibig_per)+parseFloat(temp?.gsis_per)+parseFloat(temp?.philhealth_per)+parseFloat(temp?.w_tax)+t_loan;
            ...
            let new_accrued = temp?.m_rate-temp?.lwopay;
            ...
            temp.accrued = new_accrued
            temp.total_deduction = t_deductions
    ...
    },[selectedEmp?.lwopay,selectedEmp?.w_tax,selectedEmp?.provident,selectedEmp?.pera,selectedEmp?.rice_subsidy,selectedEmp?.quarterly_allowance,selectedEmp?.subsistence_allowance,selectedEmp?.laundry_allowance,selectedEmp?.pagibig_per,,selectedEmp?.gsis_per,,selectedEmp?.philhealth_per,selectedEmp?.loan]);
    ...
    return (
            ...
            <Grid item xs={12}>
                    <Paper sx={{p:1,display:'flex',gap:1,background:blue[100]}}>
                        ...
                        <Grid item xs={7}>
                            ...
                            <TextField label='Monthly Rate' size="small" value = {formatWithCommas(selectedEmp?.m_rate)}InputLabelProps={{shrink:true}} InputProps={{readOnly:true}} fullWidth />
                            ...
                            <Box sx={{display:'flex',gap:1}}>
                                ...
                                <TextField label='Absences Amount' size="small" value = {selectedEmp?.lwopay} onChange={(val)=>setSelectedEmp({...selectedEmp,lwopay:val.target.value})} InputLabelProps={{shrink:true}} InputProps={{readOnly:editData?false:true}} fullWidth variant="standard"/>
                                </Box>
                            </Box>

                            <TextField label='Amount Accrued' size="small" value = {formatWithCommas(selectedEmp?.accrued)} InputLabelProps={{shrink:true}} InputProps={{readOnly:true}} fullWidth variant="standard"/>
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={6}>
                                        <Box sx={{display:'flex',gap:1,justifyContent:'space-between'}}>
                                        <TextField label='PagIbig (PS)' size="small" value = {formatWithCommas(selectedEmp?.pagibig_per)} onChange={(val)=>setSelectedEmp({...selectedEmp,pagibig_per:val.target.value})} InputLabelProps={{shrink:true}} InputProps={{readOnly:editData?false:true}} fullWidth variant="standard"/>
                                        <TextField label='PagIbig (GS)' size="small" value = {formatWithCommas(selectedEmp?.pagibig_gov)} onChange={(val)=>setSelectedEmp({...selectedEmp,pagibig_gov:val.target.value})} InputLabelProps={{shrink:true}} InputProps={{readOnly:editData?false:true}} fullWidth variant="standard"/>
                                    </Box>
                                </Grid>
                                <Grid item xs={6}>
                                    <Box sx={{display:'flex',gap:1,justifyContent:'space-between'}}>
                                        <TextField label='GSIS (PS)' size="small" value = {formatWithCommas(selectedEmp?.gsis_per)} onChange={(val)=>setSelectedEmp({...selectedEmp,gsis_per:val.target.value})} InputLabelProps={{shrink:true}} InputProps={{readOnly:editData?false:true}} fullWidth variant="standard"/>

                                        <TextField label='GSIS (GS)' size="small" value = {formatWithCommas(selectedEmp?.gsis_gov)} onChange={(val)=>setSelectedEmp({...selectedEmp,gsis_gov:val.target.value})} InputLabelProps={{shrink:true}}  InputProps={{readOnly:editData?false:true}} fullWidth variant="standard"/>
                                    </Box>
                                </Grid>
                                <Grid item xs={6}>
                                    <Box sx={{display:'flex',gap:1,justifyContent:'space-between'}}>
                                        <TextField label='PHIC (PS)' size="small" value = {formatWithCommas(selectedEmp?.philhealth_per)} onChange={(val)=>setSelectedEmp({...selectedEmp,philhealth_per:val.target.value})} InputLabelProps={{shrink:true}} InputProps={{readOnly:editData?false:true}} fullWidth variant="standard"/>
                                        <TextField label='PHIC (GS)' size="small" value = {formatWithCommas(selectedEmp?.philhealth_gov)} onChange={(val)=>setSelectedEmp({...selectedEmp,philhealth_gov:val.target.value})} InputLabelProps={{shrink:true}} InputProps={{readOnly:editData?false:true}} fullWidth variant="standard"/>
                                    </Box>
                                </Grid>
                                <Grid item xs={12}>
                                    <Box sx={{display:'flex',gap:1,justifyContent:'space-between'}}>
                                        <TextField label='Withholding Tax' size="small" value = {selectedEmp?.w_tax} onChange={(val)=>setSelectedEmp({...selectedEmp,w_tax:val.target.value})} InputLabelProps={{shrink:true}} InputProps={{readOnly:editData?false:true}} fullWidth variant="standard"/>
                                ...
                                <Grid container item xs={6} spacing={2}>
                                    <Grid item xs={12} sx={{display:'flex',flexDirection:'row',gap:1}}>
                                        <TextField label='Total Salary' size="small" value = {formatWithCommas(selectedEmp?.accrued)} InputLabelProps={{shrink:true}} InputProps={{readOnly:true}} fullWidth variant="standard"/>
                                        <TextField label='Total Deduction' size="small" value = {formatWithCommas(selectedEmp?.total_deduction)} InputLabelProps={{shrink:true}} InputProps={{readOnly:true}} fullWidth variant="standard"/>
                                    </Grid>
                                    <Grid item xs={12} sx={{display:'flex',flexDirection:'row',gap:1}}>
        ...
        <ManualAddSelected loanType={loanType} openManualAdd = {openManualAdd} setOpenManualAdd = {setOpenManualAdd} selectedData = {selectedEmp} setSelectedEmp={setSelectedEmp} data = {data} setData = {handleUpdateData} type = 'sub'/>
