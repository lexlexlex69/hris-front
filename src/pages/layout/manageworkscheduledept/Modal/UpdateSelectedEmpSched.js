import { Box, Grid,TextField,Typography,Button,Modal,Autocomplete,Tooltip, Fade,IconButton } from '@mui/material';
...
import UndoIcon from '@mui/icons-material/Undo';// media query
...
export default function UpdateSelectedEmployeeEmpSched(props){
    ...
    const clickDate = (value) =>{
        ...
        var t_updated_arr = updatedSched.filter((el=>{
            return el.date === moment(value._d).format('YYYY-MM-DD')
        }))
        if(t_updated_arr.length>0){
            // has_sched = true;
            has_updated = true;
        }
        // updatedSched.forEach(el=>{
        ...
        if(moment(currentDate).format('MM-YYYY') === moment(navigateDate).format('MM-YYYY')){
            if(schedule.length === 0){
                if(scheduleUpdateData.length !==0){
                    return <td {...props} style={{padding:'5px'}}><span style={{fontSize:matches?'.6rem':'1rem'}} {...props}>{currentDate.date()}</span></td>
                }else{
                    return <td {...props} title='Please select tempalte for this year first' style={{padding:'5px',pointerEvents:'none'}}><span style={{fontSize:matches?'.6rem':'1rem'}} {...props}>{currentDate.date()}</span> <br/> <span style={{background: red[800],
                    borderRadius: '10px',padding: '3px',color:'#fff',fontSize:matches?'.6rem':'.7rem'}} {...other} >No Template</span></td>
                }
            }else{
                return <td {...props} style={{padding:'5px'}}><span style={{fontSize:matches?'.6rem':'1rem'}} {...props}>{currentDate.date()}</span> <br/> <span style={{background:has_updated?'#f00': '#ee782a',
                    borderRadius: '10px',padding: '3px',color:'#fff',fontSize:matches?'.6rem':'.7rem'}} {...other}>{schedule}</span></td>
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var data2 ={
                        ...
                        is_requested:props.isRequested
                    }
                    // console.log(data2)
            ...
            if (result.isConfirmed) {
                ...
                if(props.isRequested){
                    var data2 ={
                        ...
                        remove_data:moment(selectedUpdateDate).format('YYYY-MM-DD'),
                        year:moment(selectedUpdateDate).format('YYYY'),
                        is_requested:props.isRequested,
                        type:rem_type
                    }
    ...
    const handleCopyToPrevMonth = () =>{
        var prevMonths = [];
        var startMonth = parseInt(moment(navigateDate).format('M'));
        var start = startMonth;
        var count = 1;
        var f_day = moment(navigateDate).format('dddd');
        var week_days = moment.weekdays();
        var index = week_days.indexOf(f_day)
        var rem = 35-index;
        var whrs_text = '';
        while(index>0){
            whrs_text+='0;';
        ...
        for(i;i<rem;i++){
            ...
            var found = false;
            var code;
            scheduleUpdateData.forEach(el=>{
                if(i === parseInt(el.day)){
                    found = true;
                    code = el.whrs_code;
                }
            })
            updatedSched.forEach(el=>{
                if(i===parseInt(moment(el.date,'YYYY-MM-DD').format('D'))){
                    found = true;
                    code = el.whrs_code;
                }
            })
        ...
        // scheduleUpdateData.forEach((el,key)=>{
        //     if(key+1 === parseInt(el.day)){
        //         whrs_text+=el.whrs_code+';'
        //     }else{
        //         whrs_text+='0;';
        //     }
        // })
        console.log(whrs_text.split(';'))
        while(count<startMonth){
            prevMonths.push(moment(navigateDate).subtract('months',count))
            count++;
        }
        prevMonths.sort().forEach(el=>{
            console.log(moment(el).format('MMMM DD, YYYY'))
        })
        // var end = parseInt(moment(navigateDate).format('M'));
        // var start = 1;
        // for(start;start<end;start++){
        //     prevMonths.push(moment(navar))
        // }
        console.log(scheduleUpdateData)
        console.log(updatedSched)
        console.log(navigateDate)
    }
    const handleCopyToSucceedingMonth = ()=>{
        console.log(scheduleUpdateData)
        console.log(updatedSched)
        console.log(navigateDate)
    }
    return(
        ...
        <Grid container>
            <Grid item xs={12} sx={{m:2}}>
                <Box sx={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <Box>
                        ...
                        <Typography sx={{fontSize:matches?'.8rem':'auto',textAlign:matches?'right':'left'}}><CircleOutlinedIcon sx={{color:'#c4c4c4'}}/><em> Rest Day</em></Typography>
                        <Typography sx={{ml:2,fontSize:matches?'.8rem':'auto',textAlign:matches?'right':'left'}}><CircleIcon sx={{color:'#ee782a'}}/><em> Template</em></Typography>
                        <Typography sx={{ml:2,fontSize:matches?'.8rem':'auto',textAlign:matches?'right':'left'}}><CircleIcon sx={{color:'#f00'}}/><em> Updated</em></Typography>
                        </Box>
                    </Box>
                    <Box sx={{display:'flex',gap:1}}>
                        {/* <Tooltip title='Copy from previous month schedule'><IconButton className='custom-iconbutton' color='primary'><UndoIcon/></IconButton></Tooltip>
                        <Tooltip title='Copy to succeeding month schedule' onClick = {handleCopyToSucceedingMonth}><IconButton className='custom-iconbutton' color='primary'><RedoIcon/></IconButton></Tooltip> */}
                    </Box>
                </Box>
            ...
            {/* <Grid item xs={12} sx={{m:1}}>
                <Button variant='contained' startIcon={<ChevronLeftIcon/>} onClick={handleCopyToPrevMonth}>Copy to Previous Month</Button>
            </Grid> */}
            <Backdrop
                ...
                <Box sx={updateTimeStyle}>
                    ...
                    <Grid container spacing={2} sx={{p:2}}>
                        ...
                        <Autocomplete
                            ...
                            getOptionLabel={(option) => '('+option.whrs_code+') '+option.whrs_desc}
