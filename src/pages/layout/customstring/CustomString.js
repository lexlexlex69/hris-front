...
export function formatTimeWithoutPeriod(time){
    if(time){
        if(time.trim().length>0){
            ...
            Check if valid time
            */
            var is_valid = /^([0-1]?[0-9]|2[0-4]):([0-5][0-9])(:[0-5][0-9])?$/.test(moment(time.trim(), 'H:mm').format('H:mm'))
            if(is_valid){
...
export function formatLogs(time){
    var is_valid = /^([0-1]?[0-9]|2[0-4]):([0-5][0-9])(:[0-5][0-9])?$/.test(moment(time, 'H:mm').format('H:mm'))
    if(is_valid){
...
export function formatOfficeName(text){
    if(text){
        if(text.length>40){
            if(text.length>50){
                return(
                    <span style={{fontSize:'9px'}}>{text.toUpperCase()}</span>
                )
            }else{
                return(
                <span style={{fontSize:'10px'}}>{text.toUpperCase()}</span>
            )
            }
...
export function formatDatePeriod(date1,date2){
    if(moment(date1).format('MMMM') === moment(date2).format('MMMM')){
        ...
        return moment(date1).format('MMMM DD - ')+moment(date2).format('MMMM DD, YYYY');
    }
}
...
export function evaluationDaysLeft(date){
        ...
        if(moment().format('YYYY-MM-DD') >= moment(start).format('YYYY-MM-DD')){
            if(moment().format('YYYY-MM-DD') > moment(deadline).format('YYYY-MM-DD')){
                return (
                    <em>Evaluation Deadline: <span style={{color:'red',fontWeight:'bold'}}>{moment(deadline).format('MMMM DD, YYYY')} (Overdue)</span></em>
                ...
                return (
                    <em>Evaluation Deadline: <span style={{color:'red',fontWeight:'bold'}}>{moment(deadline).format('MMMM DD, YYYY')}</span></em>
                )
            }
        }else if(moment().format('YYYY-MM-DD') < moment(start).format('YYYY-MM-DD')){
            return (
                <em>Evaluation Start: <span style={{color:'green',fontWeight:'bold'}}>{moment(start).format('MMMM DD, YYYY')}</span></em>
            )
        }
...
export function formatManualInputPosition(string){
    if(string){
        if(string.includes('(')){
            ...
            return <span>
                <strong><u>{arr[0].toUpperCase()}</u></strong> <br/>
                {`${arr[1].replace(')','')}`}
            </span>
        }else{
            return <strong><u>{string?.toUpperCase()}</u></strong>
...
export function formatTwoDateToText(date1,date2){
    ...
    }else if(moment(date1).format('YYYY-MM') === moment(date2).format('YYYY-MM')){
        return `${moment(date1).format('MMMM DD')}-${moment(date2).format('DD, YYYY')}`
    }else{
        if(moment(date1).format('YYYY') === moment(date2).format('YYYY')){
            return `${moment(date1).format('MMMM DD')} - ${moment(date2).format('MMMM DD, YYYY')}`
        }else{
            return `${moment(date1).format('MMMM DD, YYYY')} - ${moment(date2).format('MMMM DD, YYYY')}`
        }
    }
}
export function formatTwoDateToTextPayroll(date1,date2){
    if(moment(date1).format('YYYY-MM-DD') === moment(date2).format('YYYY-MM-DD')){
        ...
        return (
            <span>
            {moment(date1).format('MMMM')} {moment(date1).format('DD')}-{moment(date2).format('D, YYYY')}
            </span>
...
export function formatTwoDateToTextShort(date1,date2){
    if(moment(date1).format('YYYY-MM-DD') === moment(date2).format('YYYY-MM-DD')){
        ...
        return `${moment(date1).format('MMM. DD')}-${moment(date2).format('DD, YYYY')}`
    }else{
        if(moment(date1).format('YYYY') === moment(date2).format('YYYY')){
            return `${moment(date1).format('MMM. DD')} - ${moment(date2).format('MMM. DD, YYYY')}`
        }else{
    ...
    if(val){
        if(val.trim().length>0){
            if(val.length>50){
                ...
                return <span style={{borderBottom:'solid 1px'}}>
                            {val}
                    </span>;
            }
        }else{
...
export function formatName(fname,mname,lname,extname,type){
    if(type === 1){
        return `${lname} ${formatExtName(extname)}, ${fname} ${formatMiddlename(mname)} `
    }else{
        return `${fname} ${formatMiddlename(mname)} ${lname} ${formatExtName(extname)}`
    }
}
...
export function UnderDevelopmentStat(){
    return <Alert severity="info">This module is under development. All functionality will be available soon.</Alert>
}
