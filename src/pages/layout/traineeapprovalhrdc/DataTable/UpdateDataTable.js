...
export default function UpdateDataTable(props){
    ...
    return(
            ...
            <TableBody>
                {data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((data,key)=>
                    <TableRow key={key} hover>
                        ...
                        <StyledTableCell>
                            <Checkbox disabled={data.approved?true:false} checked ={preselect.includes(data.training_shortlist_id)?true:false}  onChange={()=>handleSelectTrainee(data.training_shortlist_id)}/>
                        </StyledTableCell>
                    </TableRow>
                )}
            </TableBody>
        ...
        <Button variant='contained' sx={{m:2}} color='success' onClick = {save} className='custom-roundbutton' disabled={preselect.length === 0 ? true:false}>Save</Button>
        </Box>
