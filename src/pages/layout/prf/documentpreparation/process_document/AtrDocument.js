...
import { formatName, formatNameAbbreviation, formatPositionName } from "../../../customstring/CustomString";
...
export const AtrDocument = React.forwardRef((props, ref) => {
  ...
  const [prepBySignatories, setPrepBySignatories] = useState({ name: 'DUMMY NAME', positionDesignation: 'DUMMY POSITION DESIGNATION', positionTitle: 'DUMMY POSITION TITLE' })
  ...
  useEffect(() => {
    ...
    props.data.applicantList.forEach((i, ix) => {
      axios.post(`/api/pds/print/getPersonalInformation${i.prf_applicant_id ? `?id=${i.prf_applicant_id}&&category=${i.is_employee === 1 ? 'employee' : 'applicant'}` : ''}`)
        .then(res => {
          const personalInformation = { ...res.data.personal_information, appoint_date: JSON.parse(i.appoint_date) };
    ...
    setReportToDetails({ cname: props.data.signatories.dept_head['assigned_by'], dept: props.data.signatories.dept_head['position'] })

    var formattedPos = formatPositionName(props.data.signatories.hr['position_name'])
                  ...
                  >
                    ...
                    {editToggler && (
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, marginTop: '1rem' }}>
                        ...
                        {/* Directed to */}
                        <TextField
                          label={<Typography variant="caption">Report To</Typography>}
                          value={reportToDetails.cname}
                          onChange={(e) => setReportToDetails({ ...reportToDetails, cname: e.target.value })}
                          sx={{ '& .MuiInputBase-input': {} }}
                          variant="outlined"
                          size="small"
                        />
                        <TextField
                          label={<Typography variant="caption">Report to Department</Typography>}
                          value={reportToDetails.dept}
                          onChange={(e) => setReportToDetails({ ...reportToDetails, dept: e.target.value })}
                          sx={{ '& .MuiInputBase-input': {} }}
                          variant="outlined"
                          size="small"
                        />
                      ...
                      <>
                        <Box sx={{ marginBottom: '2rem' }}>
                          ...
                          <Box sx={{ textTransform: 'capitalize' }}>
                            {`${(item.rAddress === null || item.rAddress === undefined || item.rAddress === '' || !item.rAddress) ? '' : item.rAddress}`}
                          </Box>
                          <Box> {`${item.paddress ? item.paddress : ''}`} </Box>
                        </Box>

                        <Box sx={{ marginBottom: '1rem' }}>
                          Dear <span style={{ fontWeight: 'bold' }}> Mr./Ms. {`${item.lname}`} </span>,
                        </Box>

                        {/* Body Content */}
                        <Box sx={{ marginBottom: '1rem', textAlign: 'justify' }}>
                          We would like to inform you that you are hired as an {props.data.prfData['position_title']} under a {props.data.prfData['emp_stat']} status at the {props.data.prfData['office_dept']} ({props.data.prfData['short_name']}) with a compensation of {props.data.salaryData['formattedWords']} (Php{testphpFormatComma.format(props.data.salaryData['sgValue'])}) effective {moment(item.appoint_date[0]).format('MMMM DD, YYYY')} to {moment(item.appoint_date[1]).format('MMMM DD, YYYY')} unless sooner terminated.
                        </Box>
                        <Box sx={{ marginBottom: '2rem', textAlign: 'justify' }}>
                          Further, you are directed to report to {reportToDetails.cname}, {reportToDetails.dept}, for the specific details of your job assignment. You are expected to perform your duties with utmost degree of excellence and dedication.
                        </Box>

                        {/* CHRMD DH SIGNATORY */}
                        <Stack sx={{ marginBottom: '3rem' }}>
                          <Box sx={{ fontWeight: 'bold' }}>{prepBySignatories.name}</Box>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'end' }}>
                          <Box sx={{ width: '250px' }}>
                            ...
                            <br />
                            <span>Date:</span> _________________________________
                          </Box>
                        </Box>
                      </>
                    )}
