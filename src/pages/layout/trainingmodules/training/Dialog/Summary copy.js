...
import {Grid,Typography,TextField,Box,Paper,FormControl,InputLabel,Input,InputAdornment,Chip,Stack,Avatar,Button,IconButton,Tooltip,Dialog,AppBar, Toolbar} from '@mui/material';
...
import { addTrainingDetails } from '../TrainingRequest';
...
export default function Summary(props){
    ...
    const handleUpdateRemainingSlot = (value)=>{
        ...
        //     console.log(remaining)
        // }
        // var val = value.target.value?parseInt(value.target.value):0
        // var remaining = val+parseInt(remainingSlot)
    ...
    return(
        ...
        <Grid container spacing={2}>
            ...
            <Grid item xs={12} lg={6} md={6}>
                ...
                <Grid item xs={12}>
                    <Grid item xs={12}>
                    <TextField label = 'Date' type = 'date' fullWidth InputLabelProps={{shrink:true}} value = {nominationDeadline} onChange = {handleNominationDeadline} required/>
                    </Grid>
                </Grid>
            ...
            <Grid item xs={12} sx={{display:'flex',flexDirection:'row',justifyContent:'space-between',mt:1}}>
                ...
                <Typography sx={{color:orange[800],fontWeight:'bold'}}>Slot: {remainingSlot}</Typography>
            </Grid>
            <Grid item xs={12}>
            <Paper>
                            ...
                            deptInfo.map((row,key)=>
                                <TableRow key ={key}>
                                    ...
                                    <StyledTableCell component="th" scope="row" align="right">
                                        ...
                                        <TextField label={row.dept_name+' Slot'} type='number' value ={row.slot} onChange = {(value)=>handleSetSlot(value,key,row.total_selected)} onFocus={(value)=>handleUpdateRemainingSlot(value)} InputProps={{inputProps: {inputMode: 'numeric', pattern: '[0-9]*',min:0}}} disabled = {row.total_selected === 0 ? true:false}/>
                                    </StyledTableCell>
                                </TableRow>
                            )
                        }
