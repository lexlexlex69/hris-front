...
import UpdateCOC from './Modal/UpdateCOC';

const Transition = React.forwardRef(function Transition(props, ref) {
  return <Slide direction="up" ref={ref} {...props} />;
});

export default function ManageCOC(){
    // media query
    ...
    const handleSetUpdateExpiration = (val)=>{
        setUpdatedExpiration(val.target.value)
    }
    const handleShowExpirationUpdate = () =>{
        setUpdateExpiration(true)
    }
    const handleCloseExpirationUpdate = () => {
        setUpdateExpiration(false)
    }
    const handleSaveExpirationUpdate = () =>{
        
        if(updatedExpiration){
            // console.log(moment(updatedExpiration).add(1,'months').date(0))
            var temp = {...employeeInfoReview}
            temp.data.expiration = moment(updatedExpiration).add(1,'months').date(0).format('YYYY-MM-DD')
            setEmployeeInfoReview(temp)
            setUpdateExpiration(false)
            setUpdatedExpiration(null)
        }else{
            Swal.fire({
                icon:'error',
                title:'Oops...',
                html:'Please select month of expiration'
            })
        }
                ...
                <Fade in={!isLoading}>
                    <Grid container>
                        {/* <Grid item xs={12} sm={12} md={12} lg={12} sx={{margin:'0 0 10px 0'}}>
                            <ModuleHeaderText title ='Manage COC'/>
                        </Grid> */}
                        <Grid item xs={12}>
                            <Box sx={{display:'flex',flexDirection:'row',justifyContent:'flex-end',gap:1}}>
                                <Button variant='contained' onClick ={()=> setAddModal(true)} color='success' className=' custom-roundbutton' startIcon={<AddIcon/>}>Add</Button>
                                <Button variant='contained' onClick ={()=> setOpenUpdate(true)} color='primary' className=' custom-roundbutton' startIcon={<SettingsIcon/>}>Update</Button>
                            </Box>
                        </Grid>
                                        ...
                                        <TableBody>
                                            {
                                                ...
                                                cocApplicationHistory.map((row,key)=>
                                                    <TableRow key = {key}>
                                                        ...
                                                        <TableCell>
                                                            <em><span style={{color:row.status === 'APPROVED'?'green':row.status === 'FOR APPROVAL'?'blue':'red'}}>
                                                            {row.status}
                        ...
                        >
                            ...
                            <Grid container sx={{pl:2,pr:2,pb:2}}>
                                ...
                                <Grid item xs={12} sx={{mt:1,display:'flex',flexDirection:'row',alignItems:'center',gap:1}}>
                                    <Typography sx={{color:red[800]}}>Expiration: {moment(employeeInfoReview.data.expiration).format('MMMM YYYY')}</Typography>
                                    {
                                        !updateExpiration
                                        ?
                                        <Tooltip title='Update Expiration'><IconButton color='success' className='custom-iconbutton' size='small' onClick={handleShowExpirationUpdate}><EditIcon size='small'/></IconButton></Tooltip>
                                        :
                                        ''
                                    }
                                    
                                    {
                                        ...
                                        ?
                                        <Box sx={{display:'flex',gap:1}}>
                                        <TextField type='month' value = {updatedExpiration} onChange={handleSetUpdateExpiration} size='small'/>
                                        <Button variant='contained' color='success' onClick={handleSaveExpirationUpdate}>Save</Button>
                                        <Button variant='contained' color='error' onClick={handleCloseExpirationUpdate}>Cancel</Button>
                                        </Box>
                                        :
                                        ''
                                    }
                                ...
                                <Grid item xs={12}>
                                    {
                                        ...
                                        :
                                        <PrintFormCOC2History employeeInfo={employeeInfoReview} dateEarned={dateEarnedReview} hours ={hoursReview}/>
                                    }
                                </Grid>
                        ...
                        <SmallModal open = {openUpdate} close = {()=>setOpenUpdate(false)} title ='Updating COC Balance'>
                            <UpdateCOC/>
                        </SmallModal>
