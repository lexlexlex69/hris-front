...
const Education = ({ arr, setState }) => {
    ...
    useEffect(async () => {
        ...
        // console.log('sortedListUpdated', sortedListUpdated)
                                            ...
                                            <>
                                                <TableRow display='flex' sx={{ bgcolor: 'warning.main' }}>
                                                    ...
                                                    <TableCell display='flex'>
                                                        {toUpdatedBackend.filter(y => parseInt(y.id) === parseInt(toUpdated[items][index].row_index))[0]?.elevel}
                                                    </TableCell>
                                                    <TableCell display='flex'>
                                                        {toUpdatedBackend.filter(y => parseInt(y.id) === parseInt(toUpdated[items][index].row_index))[0]?.nschool}
                                                    </TableCell>
                                                    <TableCell display='flex'>
                                                        {toUpdatedBackend.filter(y => parseInt(y.id) === parseInt(toUpdated[items][index].row_index))[0]?.degreecourse}
                                                    </TableCell>
                                                    <TableCell display='flex'>
                                                    ...
                                                    <TableCell display='flex'>
                                                        {toUpdatedBackend.filter(y => parseInt(y.id) === parseInt(toUpdated[items][index].row_index))[0]?.gradelevel}
                                                    </TableCell>
                                                    <TableCell display='flex'>
                                                        {toUpdatedBackend.filter(y => parseInt(y.id) === parseInt(toUpdated[items][index].row_index))[0]?.yeargrad}
                                                    </TableCell>
                                                    <TableCell display='flex'>
                                                        {toUpdatedBackend.filter(y => parseInt(y.id) === parseInt(toUpdated[items][index].row_index))[0]?.honor}
                                                    </TableCell>
                                                    <TableCell display='flex'>
                                                    </TableCell>
                                                </TableRow>
                                                    ...
                                                    {toUpdated[items]?.map((x, i) => (
                                                        <>
                                                            ...
                                                            <TableCell sx={{ verticalAlign: 'top' }}>
                                                                <Typography variant="body2" color="warning.main">[{x.table_field === 'nschool' ? 'NAME OF SCHOOL' : x.table_field === 'degreecourse' ? 'BASIC EDUCATION/DEGREE/COURSE' : x.table_field === 'elevel' ? 'LEVEL' : x.table_field === 'datefrom' ? 'DATE FROM' : x.table_field === 'dateto' ? 'DATE TO' : x.table_field === 'gradelevel' ? 'HIGHEST LEVEL/UNITS EARNED' : x.table_field === 'yeargrad' ? 'YEAR GRADUATED' : x.table_field === 'honor' ? 'SCHOLARSHIP/ACADEMIC HONORS RECEIVED' : ''}]</Typography>
