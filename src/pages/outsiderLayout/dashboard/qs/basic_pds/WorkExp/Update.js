...
const Update = ({ trainings, setTrainings, handleClose, total, setTotal, item }) => {
    ...
    const [isPresent, setIsPresent] = useState(moment(item?.dateto, "YYYY-MM-DD").format("YYYY-MM-DD") === 'Invalid date' ? true : false)
    const [inputState, setInputState] = useState({
        datefrom: item.datefrom,
        dateto: moment(item.dateto, "YYYY-MM-DD").format("YYYY-MM-DD"),
        positiontitle: item.positiontitle,
        agency: item.agency,
        salary: item.salary,
        salgrade: item.salgrade,
        status: item.status,
        govt: item.govt,
        appt_id: item.appt_id === null || item.appt_id === 'null' || item.appt_id === undefined ? '' : item.appt_id,
        file_path: ''
    })
        ...
        if (!inputState.file_path) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let res = await axios.post(`/api/recruitment/applicant/pds/WorkExp/updateWorkExp`, formData)
                    Swal.close()
                    handleCloseBackdrop()
