...
const Update = ({ trainings, setTrainings, handleClose, total, setTotal, item }) => {
    ...
    const [others, setOthers] = useState(item.typeLD === 'MANAGERIAL' || item.typeLD === 'SUPERVISORY' || item.typeLD === 'TECHNICAL' ? false : true)
    const [othersData, setOthersData] = useState(item.typeLD === 'MANAGERIAL' || item.typeLD === 'SUPERVISORY' || item.typeLD === 'TECHNICAL' ? '' : item.typeLD)
        ...
        if (!inputState.file_path) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let res = await axios.post(`/api/recruitment/applicant/pds/Trainings/updateTrainings`, formData)
                    Swal.close()
                    handleCloseBackdrop()
                    if (res.data.status === 200) {
                        let tempTrainings = trainings.map(x => x.id === item.id ? {
                            ...x,
        ...
        <Container >
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: 'calc(100vh - 66px)' }}>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1, mt: 2 }}>
                    <ApplicantPdsSelect componentTitle='TITLE OF LEARNING AND DEVELOPMENT INTERVENTIONS/TRAINING PROGRAMS' optionTitle='title_of_training' title='title' size='small' url='/api/pds/trainings/add/AutoComplete' setTitle={setInputState} defaultValue={inputState?.title} />
